#!/usr/bin/env python3
"""
NAO Phase × Lead-Week Conditional EOF Computation for T2M
=========================================================
Computes EOF bases of ERA5 2m Temperature anomalies, binned by (NAO phase, lead week).
Uses the CPC monthly NAO index with a 1-month lag to prevent data leakage.

Output: nao_t2m_eof_bases.pt containing:
  - eof_bases[(phase, lead)]: {eofs: [K, 181, 360], eigenvalues: [K]}
  - climatology: [181, 360]
  - phase_counts: dict of sample counts per (phase, lead) category

Usage:
  python compute_nao_t2m_eofs.py --data_dir /path/to/dataprocess --start_year 1999 --end_year 2020
"""
...
def main(args):
    ...
    print(f"=== NAO Phase × Lead-Week EOF Computation (T2M) ===")
    ...
    for phase in range(3):
        phase_indices = np.where(nao_phases == phase)[0]
    ...
    for phase in range(3):
        for lead in range(4):
            ...
            print(f"  NAO {phase_names[phase]:>8} Lead {lead}: {n_samples:>4} samples, top-3 var: {var_exp[:3].sum():.1f}%{fallback}")
    ...
    output_path = os.path.join(ml_model_dir, "nao_t2m_eof_bases.pt")
