...
Data Source: mjo_wave_spatial_*.zarr (Generated by extract_mjo_wave.py)
...
def drop_nondim_coords(obj):
    extra_coords = [name for name in obj.coords if name not in obj.dims]
    if extra_coords:
        obj = obj.drop_vars(extra_coords, errors='ignore')
    return obj
...
def process_year(year, ds_mjo_full, output_dir=OUTPUT_DIR, overwrite=False):
    ...
    da_mjo = drop_nondim_coords(ds_mjo_slice['mjo_wave']).compute()
        ...
        for w in range(4):
            ...
            try:
                ...
                if len(chunk.time) < 1:
                    ...
                    w_mean = drop_nondim_coords(chunk.mean(dim='time').squeeze())
                    weeks.append(w_mean)
            except Exception:
    ...
    if len(processed_data) > 0:
        ...
        ds_final['mjo_wave'].attrs = {
            ...
            'source': 'NOAA PSL OLR (causal MJO-like filtered anomalies)'
        }
...
if __name__ == "__main__":
    ...
    ds_list = []
    for path in mjo_files:
        ds = xr.open_zarr(path, consolidated=False)
        ds = drop_nondim_coords(ds)
        if 'mjo_wave' in ds:
            ds['mjo_wave'] = drop_nondim_coords(ds['mjo_wave'])
        ds_list.append(ds)

    ds_mjo_full = xr.concat(ds_list, dim='time', data_vars='minimal', coords='minimal', compat='override').sortby('time')
    time_index = pd.Index(ds_mjo_full.time.values)
    if time_index.has_duplicates:
        keep_mask = ~time_index.duplicated(keep='last')
        dropped = int((~keep_mask).sum())
        print(f"  Found {dropped} duplicate daily timestamps across MJO archives. Keeping the latest copy.")
        ds_mjo_full = ds_mjo_full.isel(time=np.flatnonzero(keep_mask))
