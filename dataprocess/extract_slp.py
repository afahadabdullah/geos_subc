import xarray as xr
import gcsfs
import os
import pandas as pd
import numpy as np
import dask
import argparse

def process_era5_slp_yearly(start_year=1999, end_year=2021, output_base_dir="/home1/11353/afahad/geos_subc/dataprocess/era5_slp"):
    # Define the GCS path to the Zarr store
    zarr_path = 'gs://gcp-public-data-arco-era5/ar/1959-2022-6h-512x256_equiangular_conservative.zarr'
    
    print(f"Connecting to {zarr_path}...")
    
    # Open the dataset lazily with Dask
    try:
        ds = xr.open_zarr(zarr_path, chunks={'time': 240, 'longitude': 256, 'latitude': 256}, consolidated=True)
        print("Dataset opened successfully.")
    except Exception as e:
        print(f"Error opening dataset: {e}")
        return

    # Define variable names (ERA5 Sea Level Pressure)
    var_name = 'mean_sea_level_pressure'
    
    # Fallback to 'msl' if 'mean_sea_level_pressure' isn't explicitly named
    if var_name not in ds:
        if 'msl' in ds:
            var_name = 'msl'
        else:
            print(f"Error: {var_name} not found in ARCO dataset. Available vars: {list(ds.data_vars)}")
            return
        
    # Define GEOS 1-degree target grid
    target_lat = np.linspace(-90, 90, 181)
    target_lon = np.linspace(0, 359, 360)
    
    # Ensure output directory exists
    os.makedirs(output_base_dir, exist_ok=True)
    
    for year in range(start_year, end_year + 1):
        output_path = os.path.join(output_base_dir, f"era5_slp_{year}.zarr")
        
        if os.path.exists(output_path):
            print(f"Skipping {year}, file already exists at {output_path}")
            continue
            
        print(f"\n--- Processing Year: {year} ---")
        
        try:
            # Slicing time for the year using robust datetime slice
            ds_year = ds.sel(time=slice(f"{year}-01-01", f"{year}-12-31"))
            
            if len(ds_year.time) == 0:
                print(f"Warning: No data found for {year} in ARCO-ERA5 bucket. Skipping.")
                continue
            
            # Extract SLP (no level dimension needed for sea level pressure)
            slp = ds_year[var_name].rename('slp')
            
            # Combine into a temporary dataset (just a single variable here)
            ds_subset = slp.to_dataset()
            
            # 2. Daily Mean Calculation
            print(f"Calculating daily means for {year}...")
            # We resample by '1D' (day)
            ds_daily = ds_subset.resample(time='1D').mean()
            
            # 3. Interpolation to GEOS 1-degree grid
            print(f"Interpolating to GEOS 1-degree (181x360)...")
            ds_interp = ds_daily.interp(latitude=target_lat, longitude=target_lon, method='linear')
            
            # 4. Save to Zarr
            print(f"Saving to {output_path}...")
            # Rechunk to ensure uniform sizes for Zarr
            ds_interp = ds_interp.chunk({'time': -1, 'latitude': 181, 'longitude': 360})
            
            # Using synchronous scheduler for stability during save
            with dask.config.set(scheduler='synchronous'):
                ds_interp.to_zarr(output_path, mode='w')
            
            print(f"Successfully saved {year}.")
            
        except Exception as e:
            print(f"Error processing {year}: {e}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process ARCO-ERA5 mean_sea_level_pressure to daily 1-degree GEOS grid.")
    parser.add_argument("--start_year", type=int, default=1999)
    parser.add_argument("--end_year", type=int, default=2021)
    parser.add_argument("--output_dir", type=str, default="/home1/11353/afahad/geos_subc/dataprocess/era5_slp")
    args = parser.parse_args()
    
    process_era5_slp_yearly(start_year=args.start_year, end_year=args.end_year, output_base_dir=args.output_dir)
