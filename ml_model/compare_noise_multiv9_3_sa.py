#!/usr/bin/env python3
"""
South Asia noise-strategy comparison for the multi-target v9.3 flow model.
...
loads the v9.3 local/global model class, and writes side-by-side GEOS / random / EOF-LHS
metrics to CSV.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare SA multi-v9.3 local/global noise strategies.")
...
def validate_current_sa_v9_config(config, output_dir):
    ...
    if zero_geos:
        raise ValueError("v9.3 SA is the GEOS-conditioned residual-T2M workflow; zero_geos_condition must be False")
