#!/usr/bin/env python3
"""
Build normalization statistics for the flow_finalv1_global model.

All predictor and target bounds are calculated on the full 181x360 global grid.
...
from dataset_flow_stats_finalv1_global import S2SHybridDataset
...
def scan_target_t2m_residual(
    ...
    domain_label = (
        target_domain_bounds.get("label", target_domain)
    ...
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=0)
    with torch.no_grad():
        for i, batch in enumerate(tqdm(loader, desc="Global T2M residual stats")):
            ...
            if i == 0:
                print("\n--- GLOBAL RESIDUAL BATCH 0 DIAGNOSTICS ---")
...
def calculate_stats(data_root, out_path, start_year, end_year, batch_size, target_domain, target_domain_bounds):
    ...
    print("\n==================================")
    print(f"Calculated flow_finalv1_global Global Bounds ({start_year}-{end_year})")
    print("==================================")
    for key, value in bounds.items():
    ...
    print(f"\nSaved flow_finalv1_global stats to {out_path}!")
...
def main():
    ...
    parser.add_argument("--out", type=str, default="ml_model/flow_finalv1_global_stats.pt")
    ...
    parser.add_argument("--target_domain", type=str, default="global")
    ...
    parser.add_argument("--domain_label", type=str, default=None)
    args = parser.parse_args()
    target_domain_bounds = None
    supplied_bounds = (args.lat_min, args.lat_max, args.lon_min, args.lon_max)
    if any(value is not None for value in supplied_bounds):
        if not all(value is not None for value in supplied_bounds):
            parser.error("lat_min, lat_max, lon_min, and lon_max must be supplied together")
        target_domain_bounds = {
            "label": args.domain_label or args.target_domain,
