...
from flow_matching_multi_v9_3 import FlowMatchingModel, CustomFlowMatcher
...
def select_geos_condition(x_geos, lead_idx, lead_selective_geos=True):
    ...
    v9.3 uses only the GEOS forecast lead matching each training sample. Set
...
def train(args, accelerator):
    ...
    if accelerator.is_main_process:
        ...
        if bool(config.get("plot_topography", True)):
            print(
                f"   [Topo Diagnostic]   : "
                f"{os.path.join(output_dir, 'topography_v9_3_diagnostic.png')}"
            )
        print(
    ...
    if accelerator.is_main_process:
        ...
        print(f"   --- v9.3 Conditioning Upgrades ---")
        print(
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_3")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_v9_3.csv")
    component_log_file = os.path.join(output_dir, "training_components_v9_3.csv")
    metrics_log_file = os.path.join(output_dir, "validation_metrics_v9_3.csv")
