#!/usr/bin/env python3
"""
Generate South Asia multi-v9.3 ensemble forecasts exactly like v9 test mode.

Unlike generate_forecast_zarr_multiv9_3_sa.py, this script creates the full
EOF-LHS/random mixed ensemble noise for each batch before chunking the ODE solve.
That matches train_flow_multiv9_3.run_full_test_suite_multi and avoids changing
...
from compare_noise_multiv9_3_sa import (
    decode_multi,
    load_noise_context,
    resolve_checkpoint,
...
from flow_matching_multi_v9_3 import CustomFlowMatcher, FlowMatchingModel
from train_flow_multiv9_3 import (
    euler_solve_chunked,
...
DEFAULT_OUT_DIR = "dataprocess/gen_multiv9_3_sa_55e100e_0n40n_junjul_testmode_e100_s50"


def parse_args():
    parser = argparse.ArgumentParser(description="Generate multi-v9.3 SA forecasts to yearly Zarr stores.")
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv9_3.yaml")
...
def write_year(year, args, config, model, flow_matcher, device, checkpoint_path, checkpoint_meta, noise_context, deadline=None):
    ...
    seen_inits = 0
    pbar = tqdm(loader, desc=f"Generate v9.3 SA {year}")
...
def main():
    ...
    print("\n" + "=" * 88)
    print("South Asia v9.3 forecast Zarr generation (test-mode equivalent)")
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        if not completed:
            ...
            break
    if all_complete:
        print("✅ Requested v9.3 SA Zarr generation range is complete.")
    else:
        print("⏸️ Requested v9.3 SA Zarr generation range is incomplete and can be resumed.")
