#!/bin/bash
#SBATCH -J SA_flow_v9_4
#SBATCH -o ml_output_flowmulti_v9_4_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v9_4_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v9.4 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv9_4.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv9_4.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv9_4.yaml"
...
CONFIG_INIT_CHECKPOINT=$(python -c "import yaml; c=yaml.safe_load(open('$CONFIG_PATH')); print(c.get('finetune_init_checkpoint') or '')")
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_4_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ -z "$CONFIG_ELEVATION_FILE" ] || [ ! -f "$CONFIG_ELEVATION_FILE" ]; then
    echo "❌ Required v9.4 elevation file is missing: $CONFIG_ELEVATION_FILE"
    exit 1
fi
if [ ! -f "$CKPT_FILE" ] && { [ -z "$CONFIG_INIT_CHECKPOINT" ] || [ ! -f "$CONFIG_INIT_CHECKPOINT" ]; }; then
    echo "❌ No v9.4 resume checkpoint exists and the required v9.3 initialization checkpoint is missing:"
    echo "   $CONFIG_INIT_CHECKPOINT"
    exit 1
fi
...
echo "🎯 Fine-tune initialization: $CONFIG_INIT_CHECKPOINT"
...
python ml_model/inspect_topography_v9_4.py \
    --file "$CONFIG_ELEVATION_FILE" \
    --variable "GLDAS_elevation" \
    --output "$OUTPUT_DIR/topography_v9_4_preflight.png"
...
echo "🔥 Launching Flow Matching Multi-Target v9.4 Training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_multiv9_4.py --config "$CONFIG_PATH" \
    --epochs-per-run 5
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_flowmultiv9_4.sh"
    else
...
echo "🏁 South Asia Multi-v9.4 local/global residual-T2M training job finished at $(date)"
