#!/usr/bin/env python3
"""
South Asia noise-strategy comparison for the multi-target v9.5b flow model.
...
loads the v9.5b local/global model class, and writes side-by-side GEOS / random / EOF-LHS
metrics to CSV.
"""
...
from dataset_flow_multi_v9_5b import GEOS_CONDITION_STATISTICS, S2SHybridDataset
...
EXPECTED_OUTPUT_DIR = "ml_output_flowmulti_v9_5b_sa_55e100e_0n40n_noisectx_t2mres"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare SA multi-v9.5b local/global noise strategies.")
...
def validate_current_sa_v9_config(config, output_dir):
    ...
    geos_statistics = tuple(config.get("geos_condition_statistics") or ())
    ...
    if zero_geos:
        raise ValueError("v9.5b SA is the GEOS-conditioned residual-T2M workflow; zero_geos_condition must be False")
    ...
    if geos_statistics != GEOS_CONDITION_STATISTICS:
        raise ValueError(
            f"Expected geos_condition_statistics={list(GEOS_CONDITION_STATISTICS)}, "
            f"got {list(geos_statistics)}"
        )
    if not bool(config.get("use_geos_temporal_encoder", False)):
        raise ValueError("v9.5b requires use_geos_temporal_encoder=True")
...
def resolve_t2m_residual_bounds(config):
    t2m_target_mode = str(config.get("t2m_target_mode", "absolute")).lower()
    ...
    if t2m_target_mode == "geos_residual":
        ...
        if residual_bounds is None:
            raise KeyError(f"{stats_path} is missing target_t2m_residual_raw")
...
def decode_multi(pred_norm, batch, device, t2m_target_mode="absolute", t2m_residual_min=-20.0, t2m_residual_max=20.0):
    ...
    if t2m_target_mode == "geos_residual":
        residual = ((pred_t2m + 1.0) / 2.0) * (t2m_residual_max - t2m_residual_min) + t2m_residual_min
        ...
        t2m = geos_t2m_lead.unsqueeze(1) + residual
    else:
