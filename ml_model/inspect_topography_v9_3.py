#!/usr/bin/env python3
"""Preflight the GLDAS elevation file before launching expensive v9.3 training."""
...
from static_geography_v9_3 import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inspect GLDAS topography schema and verify the v9.3 South Asia crop."
    )
    ...
    parser.add_argument(
        "--output",
        default="ml_output_flowmulti_v9_3_sa_55e100e_0n40n_noisectx_t2mres/"
        "topography_v9_3_preflight.png",
...
def main():
    ...
    print("=" * 88)
    print("v9.3 GLDAS TOPOGRAPHY PREFLIGHT")
    print(f"File: {path}")
    ...
    axes[0].set_title("GLDAS elevation on v9.3 grid (m)")
    ...
    fig.suptitle(
        f"v9.3 topography preflight | {os.path.basename(path)}\n"
    ...
    plt.close(fig)
    print(f"\nPASS: topography is compatible with v9.3")
