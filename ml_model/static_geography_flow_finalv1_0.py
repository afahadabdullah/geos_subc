"""Build and validate the file-free/static geography inputs used by flow_finalv1.0."""
...
def _load_land_mask(data_dir, lats, lons, mask_cache_path=None):
    ...
    if not candidates:
        warnings.warn(
            "No SSS Zarr was found for the flow_finalv1.0 land mask. The static land channel "
...
def _load_elevation(config, lats, lons):
    ...
    if elevation_path is None:
        ...
        if configured and bool(config.get("require_elevation_file", False)):
            raise FileNotFoundError(
                f"Configured flow_finalv1.0 elevation file was not found: {configured}"
            )
        warnings.warn(
            "No elevation/orography file is available for flow_finalv1.0. Using an explicit zero "
    ...
    if normalization != "minmax":
        raise ValueError(
            f"flow_finalv1.0 elevation_normalization must be 'minmax', got {normalization!r}."
        )
...
def _save_diagnostic(channels, lats, lons, path, metadata):
    ...
    fig.suptitle(
        f"flow_finalv1.0 static geography alignment | elevation={metadata['elevation']['source']}",
        fontsize=12,
    )
...
def _save_topography_diagnostic(channels, elevation_m, lats, lons, path, metadata):
    ...
    fig.suptitle(
        "flow_finalv1.0 South Asia static topography alignment\n"
...
def load_or_build_static_geography(config, lats, lons, output_dir=None):
    ...
    artifact_path = _resolve_path(
        config.get("static_geography_file", "ml_model/static_geography_flow_finalv1_0_sa.pt")
    )
    ...
    if output_dir and bool(config.get("plot_static_geography", True)):
        diagnostic_path = os.path.join(output_dir, "static_geography_flow_finalv1_0_alignment.png")
        _save_diagnostic(channels.numpy(), lats, lons, diagnostic_path, metadata)
    if output_dir and bool(config.get("plot_topography", True)):
        topography_path = os.path.join(output_dir, "topography_flow_finalv1_0_diagnostic.png")
