#!/bin/bash
#SBATCH -J SA_flow_v9_5b
#SBATCH -o ml_output_flowmulti_v9_5b_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v9_5b_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v9.5b local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv9_5b.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv9_5b.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv9_5b.yaml"
...
CONFIG_GEOS_TEMPORAL=$(python -c "import yaml; c=yaml.safe_load(open('$CONFIG_PATH')); print(c.get('use_geos_temporal_encoder'))")
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_5b_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to train v9 without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
if [ "$CONFIG_GEOS_STATS" != "mean,std,q10,q90,member_count" ]; then
    echo "❌ Refusing unexpected v9.5b GEOS statistics: $CONFIG_GEOS_STATS"
    exit 1
fi
if [ "$CONFIG_GEOS_TEMPORAL" != "True" ]; then
    echo "❌ v9.5b requires use_geos_temporal_encoder=True"
    exit 1
fi
if [ -n "$CONFIG_EXTERNAL_INIT" ]; then
    echo "❌ v9.5b must train from scratch; external initialization is configured: $CONFIG_EXTERNAL_INIT"
    exit 1
fi
if [ -z "$CONFIG_ELEVATION_FILE" ] || [ ! -f "$CONFIG_ELEVATION_FILE" ]; then
    echo "❌ Required v9.5b elevation file is missing: $CONFIG_ELEVATION_FILE"
    exit 1
fi
...
echo "🎯 GEOS temporal evolution encoder: $CONFIG_GEOS_TEMPORAL"
echo "🎯 Initialization: scratch (or resume this v9.5b output directory only)"
...
python ml_model/inspect_topography_v9_5b.py \
    --file "$CONFIG_ELEVATION_FILE" \
    --variable "GLDAS_elevation" \
    --output "$OUTPUT_DIR/topography_v9_5b_preflight.png"
...
# new ensemble-statistic channel layout diverges from v9.3.
echo "🧪 Verifying v9.5b targets and GEOS ensemble-statistic conditioning..."
python ml_model/inspect_geos_conditioning_v9_5b.py \
    --config "$CONFIG_PATH" \
...
# Preserve the original v9.3/v9.5b chaining behavior: 30 epochs per allocation.
...
echo "🔥 Launching Flow Matching Multi-Target v9.5b Training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_multiv9_5b.py --config "$CONFIG_PATH" \
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" \
            "cd $PWD && sbatch ml_model/submit_train_flowmultiv9_5b.sh"
    else
...
echo "🏁 South Asia Multi-v9.5b local/global residual-T2M training job finished at $(date)"
