...
from flow_matching_multi_v9_4 import FlowMatchingModel, CustomFlowMatcher
from static_geography_v9_4 import STATIC_CHANNEL_NAMES, load_or_build_static_geography
from extreme_sampling_v9_4 import (
    CATEGORY_NAMES,
    ExtremeBalancedBatchSampler,
    ExtremeSamplingDataset,
    build_or_load_extreme_artifact,
    select_extreme_validation_indices,
)
import noise_utils
...
def geos_condition_channel_count(lead_selective_geos=True, geos_member_count=1, geos_variable_count=2, geos_lead_count=4):
    ...
    return int(geos_member_count) * int(geos_variable_count) * lead_count
...
def select_geos_condition(x_geos, lead_idx, lead_selective_geos=True):
    ...
    v9.4 uses only the GEOS forecast lead matching each training sample. Set
    ...
    b, members, variables, _, h, w = x_geos.shape
    gather_idx = lead_idx.long().view(b, 1, 1, 1, 1, 1).expand(b, members, variables, 1, h, w)
    return x_geos.gather(3, gather_idx).squeeze(3).contiguous().view(b, members * variables, h, w)
    ...
    if t2m_target_mode == "geos_residual":
        if residual_bounds is None:
            raise KeyError(
                ...
                "in the stats file. Run calculate_global_local_stats_multi_v8.py."
            )
...
def compute_multi_variance_loss(
    ...
    if sample_weights is not None:
        sample_scale = sample_weights.view(-1, 1, 1, 1)
        sample_scale = sample_scale / (sample_scale.mean() + 1e-8)
    weighted_var_error = sample_scale * spatial_weights * temp_weights * (std_for_loss - target_scale) ** 2
    ...
    regularization = sample_scale * (var_penalty * 10.0 + identity_pull * 0.5)
...
def compute_structured_velocity_loss(
    ...
    def weighted_mse(pred, target, spatial, temporal):
        return (sample_scale * spatial * temporal * (pred - target) ** 2).mean()
    ...
    # Averaging the PR and T2M totals preserves the v9.2 two-channel MSE scale.
    return 0.5 * (variable_losses[0] + variable_losses[1]), diagnostics


def compute_extreme_physical_losses(
    endpoint_norm,
    batch,
    extreme_artifact,
    target_sqrt_min,
    ...
    t2m_residual_max,
    tail_cap=4.0,
    neighborhood_temperature_pr=1.0,
    neighborhood_scales=(3, 5),
):
    """Tail intensity and neighborhood losses on a differentiable endpoint estimate."""
    months = batch["month"].to(endpoint_norm.device).long() - 1
    leads = batch["lead_idx"].to(endpoint_norm.device).long()
    thresholds = extreme_artifact["grid_thresholds"]

    def threshold(name):
        return thresholds[name][months.cpu(), leads.cpu()].to(
            endpoint_norm.device, endpoint_norm.dtype
        ).unsqueeze(1)

    pr_norm = torch.clamp(endpoint_norm[:, 0:1], -1.0, 1.0)
    pr_sqrt = ((pr_norm + 1.0) / 2.0) * (target_sqrt_max - target_sqrt_min) + target_sqrt_min
    pred_pr = torch.clamp(pr_sqrt.square(), min=0.0)
    pred_t2m = decode_t2m_forecast_from_norm(
        endpoint_norm[:, 1:2],
        batch,
        batch["geos_ens_raw"].to(endpoint_norm.device),
        t2m_target_mode=t2m_target_mode,
        t2m_residual_min=t2m_residual_min,
        t2m_residual_max=t2m_residual_max,
    )
    target_raw = batch["target_raw"].to(endpoint_norm.device)
    target_pr = target_raw[:, 0:1]
    target_t2m = target_raw[:, 1:2]

    pr_q90, pr_q95 = threshold("pr_q90"), threshold("pr_q95")
    t2m_q05, t2m_q10 = threshold("t2m_q05"), threshold("t2m_q10")
    t2m_q90, t2m_q95 = threshold("t2m_q90"), threshold("t2m_q95")

    pr_emphasis = torch.clamp(
        torch.relu((target_pr - pr_q90) / (pr_q95 - pr_q90 + 1e-3)),
        max=float(tail_cap),
    )
    warm_emphasis = torch.clamp(
        torch.relu((target_t2m - t2m_q90) / (t2m_q95 - t2m_q90 + 1e-3)),
        max=float(tail_cap),
    )
    cold_emphasis = torch.clamp(
        torch.relu((t2m_q10 - target_t2m) / (t2m_q10 - t2m_q05 + 1e-3)),
        max=float(tail_cap),
    )

    def emphasized_smooth_l1(pred, target, emphasis):
        value = F.smooth_l1_loss(pred, target, reduction="none")
        return (value * emphasis).sum() / (emphasis.sum() + 1e-6)

    pr_tail = emphasized_smooth_l1(pred_pr, target_pr, pr_emphasis)
    warm_tail = emphasized_smooth_l1(pred_t2m, target_t2m, warm_emphasis)
    cold_tail = emphasized_smooth_l1(pred_t2m, target_t2m, cold_emphasis)

    pred_pr_flat = pred_pr.flatten(2)
    target_pr_flat = target_pr.flatten(2)
    pr_quantile = sum(
        F.smooth_l1_loss(
            torch.quantile(pred_pr_flat, quantile, dim=2),
            torch.quantile(target_pr_flat, quantile, dim=2),
        )
        for quantile in (0.90, 0.95, 0.99)
    ) / 3.0
    tail_total = (pr_tail + warm_tail + cold_tail + pr_quantile) / 4.0

    temperature = max(float(neighborhood_temperature_pr), 1e-3)
    neighborhood_parts = []
    neighborhood_diag = {}
    for threshold_name, threshold_map in (("q90", pr_q90), ("q95", pr_q95)):
        pred_event = torch.sigmoid((pred_pr - threshold_map) / temperature)
        target_event = torch.sigmoid((target_pr - threshold_map) / temperature)
        for scale in neighborhood_scales:
            scale = int(scale)
            pred_fraction = F.avg_pool2d(
                pred_event, kernel_size=scale, stride=1, padding=scale // 2
            )
            target_fraction = F.avg_pool2d(
                target_event, kernel_size=scale, stride=1, padding=scale // 2
            )
            part = F.smooth_l1_loss(pred_fraction, target_fraction)
            neighborhood_parts.append(part)
            neighborhood_diag[f"pr_{threshold_name}_n{scale}"] = part.detach()
    neighborhood_total = torch.stack(neighborhood_parts).mean()
    diagnostics = {
        "pr_tail": pr_tail.detach(),
        "warm_t2m_tail": warm_tail.detach(),
        "cold_t2m_tail": cold_tail.detach(),
        "pr_quantile": pr_quantile.detach(),
        **neighborhood_diag,
    }
    return tail_total, neighborhood_total, diagnostics
...
def train(args, accelerator):
    ...
    finetune_init_checkpoint = str(config.get("finetune_init_checkpoint", "")).strip()
    finetune_require_init = bool(config.get("finetune_require_init", True))
    extreme_balanced_sampling = bool(config.get("extreme_balanced_sampling", True))
    extreme_sampling_artifact_path = str(
        config.get("extreme_sampling_artifact", "ml_model/extreme_sampling_multiv9_4_sa.pt")
    )
    rebuild_extreme_sampling_artifact = bool(
        config.get("rebuild_extreme_sampling_artifact", False)
    )
    extreme_sampling_fractions = tuple(
        float(value)
        for value in config.get("extreme_sampling_fractions", [0.65, 0.15, 0.10, 0.10])
    )
    extreme_uniform_fraction = float(config.get("extreme_uniform_fraction", 0.0))
    extreme_sampling_correction_min = float(
        config.get("extreme_sampling_correction_min", 0.25)
    )
    extreme_sampling_correction_max = float(
        config.get("extreme_sampling_correction_max", 4.0)
    )
    extreme_sampling_seed = int(config.get("extreme_sampling_seed", 941))
    use_extreme_validation = bool(config.get("extreme_validation", True))
    extreme_validation_max_per_category = int(
        config.get("extreme_validation_max_per_category", 64)
    )
    extreme_validation_seed = int(config.get("extreme_validation_seed", 1941))
    extreme_validation_selection_weight = float(
        config.get("extreme_validation_selection_weight", 0.50)
    )
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_4")
    ...
    train_dataset = None
    extreme_artifact = None
    extreme_batch_sampler = None
    extreme_val_loader = None
    loader = None
    if not args.test:
        ...
        if extreme_balanced_sampling:
            extreme_artifact = build_or_load_extreme_artifact(
                train_dataset,
                extreme_sampling_artifact_path,
                force=rebuild_extreme_sampling_artifact,
            )
            extreme_batch_sampler = ExtremeBalancedBatchSampler(
                extreme_artifact["labels"],
                batch_size=batch_size,
                target_fractions=extreme_sampling_fractions,
                uniform_fraction=extreme_uniform_fraction,
                correction_min=extreme_sampling_correction_min,
                correction_max=extreme_sampling_correction_max,
                seed=extreme_sampling_seed,
            )
            loader = DataLoader(
                ExtremeSamplingDataset(train_dataset),
                batch_sampler=extreme_batch_sampler,
                num_workers=config.get("num_workers", 4),
                pin_memory=pin_memory,
            )
            if accelerator.is_main_process:
                summary = extreme_batch_sampler.summary()
                print(f"✅ v9.4 extreme artifact: {extreme_sampling_artifact_path}")
                print(f"   Natural counts: {summary['natural_counts']}")
                print(f"   Sample fractions: {summary['sample_fractions']}")
                print(f"   Inverse corrections: {summary['correction_weights']}")
                with open(os.path.join(output_dir, "extreme_sampling_summary.json"), "w") as handle:
        ...
        if use_extreme_validation:
            if extreme_artifact is None:
                if not os.path.exists(extreme_sampling_artifact_path):
                    raise FileNotFoundError(
                        "Extreme validation requires the training-only threshold artifact: "
                        f"{extreme_sampling_artifact_path}"
                    )
                extreme_artifact = torch.load(
                    extreme_sampling_artifact_path, map_location="cpu", weights_only=True
                )
            extreme_indices, extreme_counts = select_extreme_validation_indices(
                val_dataset_full,
                extreme_artifact,
                max_per_category=extreme_validation_max_per_category,
                seed=extreme_validation_seed,
            )
            if not extreme_indices:
                raise RuntimeError("No extreme validation samples were selected.")
            extreme_val_loader = DataLoader(
                Subset(val_dataset_full, extreme_indices),
            ...
            if accelerator.is_main_process:
                print(
                    f"✅ Fixed extreme validation subset: {len(extreme_indices)} samples "
                    f"{extreme_counts}"
                )
    ...
    if not os.path.exists(stats_file):
        raise FileNotFoundError(
            f"CRITICAL: {stats_file} missing. Please run calculate_global_local_stats_multi_v8.py first!"
        )
    ...
    # ``var_forced`` is the canonical v9.4 switch for entering variance-only
    ...
    physical_tail_loss_weight = float(config.get("physical_tail_loss_weight", 0.0))
    neighborhood_extreme_loss_weight = float(
        config.get("neighborhood_extreme_loss_weight", 0.0)
    )
    extreme_loss_start_epoch = int(config.get("extreme_loss_start_epoch", 1))
    extreme_loss_ramp_epochs = int(config.get("extreme_loss_ramp_epochs", 1))
    extreme_tail_cap = float(config.get("extreme_tail_cap", 4.0))
    extreme_neighborhood_temperature_pr = float(
        config.get("extreme_neighborhood_temperature_pr", 1.0)
    )
    extreme_neighborhood_scales = tuple(
        int(value) for value in config.get("extreme_neighborhood_scales", [3, 5])
    )
    ...
    if physical_tail_loss_weight < 0.0 or neighborhood_extreme_loss_weight < 0.0:
        raise ValueError("v9.4 physical tail and neighborhood weights must be non-negative")
    if extreme_loss_ramp_epochs < 1:
        raise ValueError("extreme_loss_ramp_epochs must be >= 1")
    if any(scale < 1 or scale % 2 == 0 for scale in extreme_neighborhood_scales):
        raise ValueError("extreme_neighborhood_scales must contain positive odd kernels")
    if not 0.0 <= extreme_validation_selection_weight <= 1.0:
        raise ValueError("extreme_validation_selection_weight must be in [0, 1]")
    ...
    if accelerator.is_main_process:
        ...
        print(f"   [GEOS Conditioning]  : {'target lead only' if lead_selective_geos else 'all 4 leads'} ({geos_channel_count} channels)")
        ...
        if bool(config.get("plot_topography", True)):
            print(
                f"   [Topo Diagnostic]   : "
                f"{os.path.join(output_dir, 'topography_v9_4_diagnostic.png')}"
            )
        print(
        ...
        print(
            f"   [v9.4 Extreme Loss] : tail={physical_tail_loss_weight:.3f}, "
            f"neighborhood={neighborhood_extreme_loss_weight:.3f}, "
            f"start={extreme_loss_start_epoch}, ramp={extreme_loss_ramp_epochs}"
        )
        if extreme_artifact is not None:
            print(
                f"   [Extreme Artifact]  : {extreme_sampling_artifact_path} "
                f"({extreme_artifact['training_year_start']}-{extreme_artifact['training_year_end']})"
            )
    ...
    if not args.test:
        if extreme_val_loader is not None:
            (
                model,
                optimizer,
                loader,
    ...
    if accelerator.is_main_process:
        ...
        if lead_selective_geos:
            print(f"     [{offset:02d}] x_geos: GEOS Precipitation Forecast (target lead only, local)")
            offset += 1
            print(f"     [{offset:02d}] x_geos: GEOS T2M Forecast (target lead only, local)")
            offset += 1
        else:
        ...
        print(f"   --- v9.4 Conditioning Upgrades ---")
        print(
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_4")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_v9_4.csv")
    component_log_file = os.path.join(output_dir, "training_components_v9_4.csv")
    metrics_log_file = os.path.join(output_dir, "validation_metrics_v9_4.csv")
    ...
    validation_metrics_header = [
        ...
        "val_noise_mse",
        "combined_crps",
        "natural_combined_crps",
        "extreme_combined_crps",
        "extreme_selection_score",
        "ml_pr_crps",
        "ml_pr_rmse",
        "ml_pr_bias",
        "geos_pr_crps",
    ...
    extreme_validation_noise_cache = {}
    ...
    finetune_source_checkpoint = ""
    finetune_source_epoch = 0
    ...
    if os.path.exists(ckpt_path):
        try:
            ...
            finetune_source_checkpoint = str(
                checkpoint.get("finetune_source_checkpoint", finetune_source_checkpoint)
            )
            finetune_source_epoch = int(
                checkpoint.get("finetune_source_epoch", finetune_source_epoch)
            )
            # Unwrap for loading
            unwrapped_model = accelerator.unwrap_model(model)
            ...
            if not args.test:
                ...
                if best_val_metric not in {
                    "mse_noise",
                    "combined_crps",
                    "extreme_composite_crps_ratio",
                }:
            ...
            if accelerator.is_main_process:
                ...
                if not args.test:
                    ...
                    if start_epoch > crps_selection_start_epoch and best_val_metric in {
                        "combined_crps",
                        "extreme_composite_crps_ratio",
                    }:
                        print(
        ...
        if not finetune_init_checkpoint:
            if finetune_require_init:
                raise ValueError(
                    "v9.4 requires finetune_init_checkpoint when no v9.4 latest checkpoint exists."
                )
            if accelerator.is_main_process:
                print("\n🚀 Starting v9.4 without a fine-tune checkpoint.\n")
        else:
            init_path = finetune_init_checkpoint
            if not os.path.isabs(init_path):
                init_path = os.path.abspath(init_path)
            if not os.path.exists(init_path):
                raise FileNotFoundError(
                    f"Required v9.3 fine-tune checkpoint was not found: {init_path}"
                )
            source = torch.load(init_path, map_location="cpu", weights_only=True)
            accelerator.unwrap_model(model).load_state_dict(source["model"], strict=True)
            for parameter in accelerator.unwrap_model(model).parameters():
                parameter.requires_grad_(True)
            finetune_source_checkpoint = init_path
            finetune_source_epoch = int(source.get("epoch", 0))
            start_epoch = 1
            best_val_loss = float("inf")
            best_val_epoch = 0
            best_val_metric = "extreme_composite_crps_ratio"
            top_models = []
            if accelerator.is_main_process:
                print("\n🌪️ Initializing v9.4 extreme fine-tuning")
                print(f"   Source checkpoint: {init_path}")
                print(f"   Source v9.3 epoch: {finetune_source_epoch}")
                print(f"   v9.3 optimizer/RNG/epoch state: intentionally not loaded")
                print(f"   v9.4 optimizer: fresh AdamW at lr={lr:.2e}")
                print("   All backbone, attention, PR/T2M velocity, and variance parameters are unfrozen.\n")
    ...
    if args.test:
        ...
        run_full_test_suite_multi(
            ...
            use_flow_variance=(
                force_variance_phase
                or loaded_is_variance_phase
                or joint_variance_start_epoch > 0
            ),
            eof_bases=eof_bases,
            nao_bases=nao_bases,
            nao_lookup=nao_lookup,
    ...
    for epoch in range(start_epoch, epochs + 1):
        if epochs_done_this_run >= max_epochs_this_run:
            ...
            break
        if extreme_batch_sampler is not None:
            extreme_batch_sampler.set_epoch(epoch)
        if (
        ...
        train_crps_steps = 0
        train_tail_loss_total = 0.0
        train_neighborhood_loss_total = 0.0
        train_extreme_steps = 0
        sampled_category_counts = torch.zeros(len(CATEGORY_NAMES), dtype=torch.long)
        if crps_loss:
            phase_label = "CRPS"
        else:
            if is_variance_phase:
        ...
        for i, batch in enumerate(pbar):
            ...
            sample_weights = batch.get("sampling_weight")
            if sample_weights is not None:
                sample_weights = sample_weights.to(device)
            if "sampling_category" in batch:
                sampled_category_counts += torch.bincount(
                    batch["sampling_category"].detach().cpu().long(),
            ...
            if crps_loss:
                ...
                if is_variance_phase:
                    ...
                    sample_weights_for_flow = (
                        sample_weights.unsqueeze(1).expand(B, E_var).reshape(-1)
                        if sample_weights is not None
                        else None
                    )
                    ...
                    sample_weights_for_flow = sample_weights
                    flow_batch_size = B
                ...
                loss_vel, velocity_diag = compute_structured_velocity_loss(
                    ...
                    sample_weights=sample_weights_for_flow,
                )
                ...
                if (
                    not is_variance_phase
                    and extreme_artifact is not None
                    and epoch >= extreme_loss_start_epoch
                    and (
                        physical_tail_loss_weight > 0.0
                        or neighborhood_extreme_loss_weight > 0.0
                    )
                ):
                    ramp = min(
                        1.0,
                        (epoch - extreme_loss_start_epoch + 1)
                        / max(1, extreme_loss_ramp_epochs),
                    )
                    endpoint_norm = x_t + (1.0 - t.view(-1, 1, 1, 1)) * v_pred
                    tail_loss, neighborhood_loss, extreme_diag = compute_extreme_physical_losses(
                        endpoint_norm=endpoint_norm,
                        batch=batch,
                        extreme_artifact=extreme_artifact,
                        target_sqrt_min=target_sqrt_min,
                        target_sqrt_max=target_sqrt_max,
                        t2m_target_mode=t2m_target_mode,
                        t2m_residual_min=t2m_residual_min,
                        t2m_residual_max=t2m_residual_max,
                        tail_cap=extreme_tail_cap,
                        neighborhood_temperature_pr=extreme_neighborhood_temperature_pr,
                        neighborhood_scales=extreme_neighborhood_scales,
                    )
                    loss = loss + ramp * (
                        physical_tail_loss_weight * tail_loss
                        + neighborhood_extreme_loss_weight * neighborhood_loss
                    )
                    train_tail_loss_total += float(tail_loss.detach().item())
                    train_neighborhood_loss_total += float(neighborhood_loss.detach().item())
                    train_extreme_steps += 1
        ...
        if accelerator.is_main_process:
            ...
            if train_extreme_steps > 0:
                print(
                    "   v9.4 extreme components "
                    f"| physical tail={train_tail_loss_total / train_extreme_steps:.4f} "
                    f"| neighborhood={train_neighborhood_loss_total / train_extreme_steps:.4f}"
                )
            if sampled_category_counts.sum() > 0:
                sampled_total = int(sampled_category_counts.sum())
                sampled_summary = {
                    CATEGORY_NAMES[index]: round(
                        int(sampled_category_counts[index]) / sampled_total, 4
                    )
                    for index in range(len(CATEGORY_NAMES))
                }
                print(f"   v9.4 sampled category fractions: {sampled_summary}")
        ...
        if accelerator.is_main_process:
            ...
            ckpt = {
                ...
                'finetune_source_checkpoint': finetune_source_checkpoint,
                'finetune_source_epoch': finetune_source_epoch,
        ...
        if use_crps_phase:
            ...
            extreme_val_result = None
            if extreme_val_loader is not None:
                extreme_val_result = run_with_isolated_rng(
                    validation_seed + 9001,
                    run_val_inference,
                    epoch, model, extreme_val_loader, flow_matcher, device, accelerator, output_dir, log_file,
                    target_sqrt_min, target_sqrt_max, geos_min, geos_max, area_weights, global_bounds,
                    is_test=False, is_fast_recon=True, use_flow_variance=use_flow_variance,
                    use_eof_lhs_noise=True,
                    validation_noise_cache=extreme_validation_noise_cache,
                    print_validation_noise_diag=False,
                    eof_bases=eof_bases, nao_bases=nao_bases, nao_lookup=nao_lookup,
                    enso_bases=enso_bases, oni_lookup=oni_lookup, mjo_df=mjo_df,
                    t2m_eof_bases=t2m_eof_bases, t2m_nao_bases=t2m_nao_bases, t2m_enso_bases=t2m_enso_bases,
                ...
                natural_geos = 0.5 * (
                    val_result["avg_geos_crps_pr"] + val_result["avg_geos_crps_t2m"]
                )
                extreme_geos = 0.5 * (
                    extreme_val_result["avg_geos_crps_pr"]
                    + extreme_val_result["avg_geos_crps_t2m"]
                )
                natural_ratio = val_result["combined_crps"] / max(natural_geos, 1e-8)
                extreme_ratio = extreme_val_result["combined_crps"] / max(extreme_geos, 1e-8)
                current_val_metric = (
                    (1.0 - extreme_validation_selection_weight) * natural_ratio
                    + extreme_validation_selection_weight * extreme_ratio
                )
            else:
            ...
            if accelerator.is_main_process:
                print(
                    f"✅ Natural CRPS Done. Combined: {val_result['combined_crps']:.4f} | "
                ...
                if extreme_val_result is not None:
                    print(
                        f"🌪️ Extreme CRPS: combined={extreme_val_result['combined_crps']:.4f} | "
                        f"PR={extreme_val_result['avg_crps_pr']:.4f} | "
                        f"T2M={extreme_val_result['avg_crps_t2m']:.4f}"
                    )
                    print(
                        f"   v9.4 selection score={current_val_metric:.6f} "
                        f"(natural/extreme model-to-GEOS ratios, "
                        f"extreme weight={extreme_validation_selection_weight:.2f})"
                    )
                is_new_best = (current_val_metric < best_val_loss)
                if is_new_best:
                    print(f"🏆 NEW BEST (v9.4 selection)! {current_val_metric:.6f} (Prev: {best_val_loss:.6f})")
                    ...
                    new_best_name = f"best_model_epoch_{epoch}_v94score_{current_val_metric:.6f}.pt"
                    ...
                    best_ckpt = {
                        ...
                        'finetune_source_checkpoint': finetune_source_checkpoint,
                        'finetune_source_epoch': finetune_source_epoch,
                        'natural_combined_crps': val_result['combined_crps'],
                        'extreme_combined_crps': (
                            extreme_val_result['combined_crps']
                            if extreme_val_result is not None else None
                        ),
                        'rng_state': capture_rng_state(train_loader_generator),
                    }
                    ...
                    if extreme_val_result is not None:
                        evt = extreme_val_result["tensors"]
                        save_val_plot(
                            epoch,
                            evt["full_pred"],
                            evt["true_target"],
                            extreme_val_result["avg_crps_pr"],
                            extreme_val_result["avg_rmse_pr"],
                            evt["geos_mean"],
                            extreme_val_result["avg_geos_crps_pr"],
                            extreme_val_result["avg_geos_rmse_pr"],
                            output_dir,
                            ai_residual=evt["ai_res"],
                            suffix="best_extreme_crps",
                            geos_single=evt["geos_single"],
                            model_single=evt["model_single"],
                            model_var=evt["model_var"],
                            full_pred_t2m=evt["full_pred_t2m"],
                            true_target_t2m=evt["true_target_t2m"],
                            geos_pred_t2m=evt["geos_mean_t2m"],
                            model_var_t2m=evt["model_var_t2m"],
                            model_crps_t2m=extreme_val_result["avg_crps_t2m"],
                            model_rmse_t2m=extreme_val_result["avg_rmse_t2m"],
                            geos_crps_t2m=extreme_val_result["avg_geos_crps_t2m"],
                            geos_rmse_t2m=extreme_val_result["avg_geos_rmse_t2m"],
                        )
                        print(f"📸 Extreme validation plot saved for Epoch {epoch}.")
                ...
                append_validation_metrics({
                    ...
                    "variance_coarse_kernel": validation_variance_coarse_kernel,
                    "combined_crps": val_result['combined_crps'],
                    "natural_combined_crps": val_result['combined_crps'],
                    "extreme_combined_crps": (
                        extreme_val_result['combined_crps']
                        if extreme_val_result is not None else ""
                    ),
                    "extreme_selection_score": current_val_metric,
...
def main():
    ...
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv9_4.yaml")
