#!/bin/bash
#SBATCH -J SA_flow_v9_5a
#SBATCH -o ml_output_flowmulti_v9_5a_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v9_5a_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v9.5a local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv9_5a.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv9_5a.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv9_5a.yaml"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_5a_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ "$CONFIG_GEOS_STATS" != "mean,std,q10,q90,member_count" ]; then
    echo "❌ Refusing unexpected v9.5a GEOS statistics: $CONFIG_GEOS_STATS"
    exit 1
fi
if [ -n "$CONFIG_EXTERNAL_INIT" ]; then
    echo "❌ v9.5a must train from scratch; external initialization is configured: $CONFIG_EXTERNAL_INIT"
    exit 1
fi
if [ -z "$CONFIG_ELEVATION_FILE" ] || [ ! -f "$CONFIG_ELEVATION_FILE" ]; then
    echo "❌ Required v9.5a elevation file is missing: $CONFIG_ELEVATION_FILE"
    exit 1
fi
...
echo "🎯 Initialization: scratch (or resume this v9.5a output directory only)"
...
python ml_model/inspect_topography_v9_5a.py \
    --file "$CONFIG_ELEVATION_FILE" \
    --variable "GLDAS_elevation" \
    --output "$OUTPUT_DIR/topography_v9_5a_preflight.png"
...
echo "🧪 Verifying v9.5a targets and GEOS ensemble-statistic conditioning..."
python ml_model/inspect_geos_conditioning_v9_5a.py \
    --config "$CONFIG_PATH" \
...
echo "🏁 South Asia Multi-v9.5a local/global residual-T2M training job finished at $(date)"
