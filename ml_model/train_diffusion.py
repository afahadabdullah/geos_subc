    ...
    for epoch in range(start_epoch, config["epochs"]):
        ...
        with torch.no_grad():
            ...
            for i_ens in range(N_SAMPLES_VAL):
                ...
                # Convert from log1p space to mm/day
                sample_precip = torch.expm1(sample_denorm.clamp(min=0.0))
        ...
        fb_target_mm = torch.expm1(fb_target.clamp(min=0.0))
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            for i_ens in range(N_MEMBERS_TEST):
                ...
                # log1p -> mm/day
                gen_mm = torch.expm1(gen.clamp(min=0.0)).clamp(min=0.0)
