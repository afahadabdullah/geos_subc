#!/bin/bash
#SBATCH -J SA_noise_v7
#SBATCH -o ml_output_noise_compare_sa_v7/compare_noise_%j.log
#SBATCH -e ml_output_noise_compare_sa_v7/compare_noise_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia v7 noise comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_noise_compare_sa_v7

CONFIG_PATH="${SA_NOISE_CONFIG:-ml_model/config_flow_multiv7.yaml}"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v7_south_asia_global_context_t2mres" ]; then
...
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to run v7 compare without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
python ml_model/compare_noise_multiv7_sa.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
...
echo "🏁 South Asia v7 noise comparison finished at $(date)"
