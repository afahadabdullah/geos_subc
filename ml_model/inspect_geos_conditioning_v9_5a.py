#!/usr/bin/env python3
"""Fail-fast comparison of v9.3 and v9.5a targets plus GEOS statistics."""
...
from dataset_flow_multi_v9_5a import (
    ...
    S2SHybridDataset as V95aDataset,
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify v9.5a GEOS statistics without changing v9.3 targets."
    )
    parser.add_argument("--config", default="ml_model/config_flow_multiv9_5a.yaml")
...
def main():
    ...
    current = make_dataset(V95aDataset, config, year)
    if len(legacy) != len(current):
        raise AssertionError(
            f"Dataset length changed: v9.3={len(legacy)}, v9.5a={len(current)}"
        )
    ...
    for key in (
        ...
        assert_close(key, v95a[key], v93[key])
    ...
    raw = v95a["geos_ens_raw"].float()
    member_count = int(raw.shape[0])
    member_count_by_lead = v95a["geos_member_count_raw"].float()
    ...
    print("PASS: v9.5a target/predictor equivalence and GEOS statistics")
    print(f"  config       : {args.config}")
    print(f"  year/index   : {year}/{sample_index}")
    print(f"  init/lead    : {v95a['year']}-{v95a['month']:02d}-{v95a['day']:02d} / {int(v95a['lead_idx']) + 1}")
    print(
