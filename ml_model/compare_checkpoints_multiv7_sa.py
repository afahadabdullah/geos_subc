#!/usr/bin/env python3
"""
South Asia v7 checkpoint sweep under pure random noise.
...
from compare_noise_multiv7_sa import (
    build_condition,
    decode_multi,
...
def save_results(results, metadata, output_dir, year, sample_tag):
    ...
    summary_path = os.path.join(output_dir, f"checkpoint_pure_noise_v7_sa_{year}_{sample_tag}_summary.csv")
    detail_path = os.path.join(output_dir, f"checkpoint_pure_noise_v7_sa_{year}_{sample_tag}_detail.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Compare SA v7 checkpoints under pure random noise.")
    ...
    dataset = S2SHybridDataset(
        ...
        normalize=True,
        preload=False,
        stats_file=config.get("stats_file", "v7_sa_global_local_stats.pt"),
    ...
    print("\n" + "=" * 96)
    print("South Asia v7 checkpoint sweep: pure random noise only")
