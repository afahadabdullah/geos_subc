#!/usr/bin/env python3
"""
South Asia noise-strategy comparison for the multi-target flow_finalv1.0 flow model.
...
loads the flow_finalv1.0 local/global model class, and writes side-by-side GEOS / random / EOF-LHS
metrics to CSV.
"""
...
from dataset_flow_finalv1_0 import GEOS_CONDITION_STATISTICS, S2SHybridDataset
...
EXPECTED_OUTPUT_DIR = "ml_output_flow_finalv1_0_sa_55e100e_0n40n_noisectx_t2mres"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare SA flow_finalv1.0 local/global noise strategies.")
...
def validate_current_sa_v9_config(config, output_dir):
    ...
    if zero_geos:
        raise ValueError("flow_finalv1.0 SA is the GEOS-conditioned residual-T2M workflow; zero_geos_condition must be False")
...
def resolve_t2m_residual_bounds(config):
    ...
    if t2m_target_mode == "geos_residual":
        stats_path = resolve_stats_path(config.get("stats_file", "flow_finalv1_0_sa_55e100e_0n40n_global_local_stats.pt"))
...
def load_noise_context(config, domain_info):
    ...
    if os.path.exists(nao_path):
        context["nao_lookup"] = noise_utils_finalv1_0.parse_nao_index(nao_path)
    if os.path.exists(oni_path):
        context["oni_lookup"] = noise_utils_finalv1_0.parse_oni_index(oni_path)
...
def main():
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc="SA noise compare")):
        ...
        for setting in settings:
            ...
            noise = noise_utils_multi_finalv1_0.mix_noise_with_random_multi(
                eof_noise,
                setting["rho_pr"],
                setting["rho_t2m"],
            )
