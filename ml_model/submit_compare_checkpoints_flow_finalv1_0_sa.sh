#!/bin/bash
#SBATCH -J SA_ckpt_flow_finalv1_0
#SBATCH -o ml_output_checkpoint_compare_sa_flow_finalv1_0/checkpoint_compare_%j.log
#SBATCH -e ml_output_checkpoint_compare_sa_flow_finalv1_0/checkpoint_compare_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia flow_finalv1.0 checkpoint comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_checkpoint_compare_sa_flow_finalv1_0

CONFIG_PATH="${SA_CKPT_CONFIG:-ml_model/config_flow_finalv1_0.yaml}"
...
python ml_model/compare_checkpoints_flow_finalv1_0_sa.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
    --epochs "$EPOCHS" \
...
echo "🏁 South Asia flow_finalv1.0 checkpoint comparison finished at $(date)"
