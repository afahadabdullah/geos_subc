#!/bin/bash
#SBATCH -J SA_finalv1_0
#SBATCH -o ml_output_flow_finalv1_0_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flow_finalv1_0_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia flow_finalv1.0 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_finalv1_0.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_finalv1_0.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_finalv1_0.yaml"
...
STATS_FILENAME=$(python -c "import yaml; print(yaml.safe_load(open('$CONFIG_PATH')).get('stats_file', 'flow_finalv1_0_sa_55e100e_0n40n_global_local_stats.pt'))")
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flow_finalv1_0_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ "$CONFIG_LAT_MIN" != "0.0" ] || [ "$CONFIG_LAT_MAX" != "40.0" ] || [ "$CONFIG_LON_MIN" != "55.0" ] || [ "$CONFIG_LON_MAX" != "100.0" ]; then
    echo "❌ Refusing unexpected release target bounds: lat=$CONFIG_LAT_MIN..$CONFIG_LAT_MAX lon=$CONFIG_LON_MIN..$CONFIG_LON_MAX"
    exit 1
fi
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to train flow_finalv1.0 without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
if [ "$CONFIG_GEOS_STATS" != "mean,std,q10,q90,member_count" ]; then
    echo "❌ Refusing unexpected flow_finalv1.0 GEOS statistics: $CONFIG_GEOS_STATS"
    exit 1
fi
if [ -n "$CONFIG_EXTERNAL_INIT" ]; then
    echo "❌ flow_finalv1.0 must train from scratch; external initialization is configured: $CONFIG_EXTERNAL_INIT"
    exit 1
fi
if [ -z "$CONFIG_ELEVATION_FILE" ] || [ ! -f "$CONFIG_ELEVATION_FILE" ]; then
    echo "❌ Required flow_finalv1.0 elevation file is missing: $CONFIG_ELEVATION_FILE"
    exit 1
fi
...
echo "🎯 Initialization: scratch (or resume this flow_finalv1.0 output directory only)"
...
python ml_model/inspect_topography_flow_finalv1_0.py \
    --file "$CONFIG_ELEVATION_FILE" \
    --variable "GLDAS_elevation" \
    --output "$OUTPUT_DIR/topography_flow_finalv1_0_preflight.png"
...
if [ ! -f "$STATS_PATH" ]; then
    echo "📊 Stats file not found. Computing flow_finalv1.0 statistics from $DATA_DIR_OVERRIDE..."
    python ml_model/calculate_global_local_stats_flow_finalv1_0.py \
    ...
    echo "✅ flow_finalv1.0 stats found at $STATS_PATH. Skipping recalculation."
fi
...
echo "🧪 Verifying flow_finalv1.0 targets and GEOS ensemble-statistic conditioning..."
python ml_model/inspect_geos_conditioning_flow_finalv1_0.py \
    --config "$CONFIG_PATH" \
...
echo "🔥 Launching flow_finalv1.0 training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_finalv1_0.py --config "$CONFIG_PATH" \
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" \
            "cd $PWD && sbatch ml_model/submit_train_flow_finalv1_0.sh"
    else
...
echo "🏁 South Asia flow_finalv1.0 local/global residual-T2M training job finished at $(date)"
