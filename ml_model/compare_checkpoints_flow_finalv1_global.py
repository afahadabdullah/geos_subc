#!/usr/bin/env python3
"""
Global flow_finalv1_global checkpoint sweep under pure random noise.
...
from dataset_flow_finalv1_global import S2SHybridDataset
...
def save_results(results, metadata, output_dir, year, sample_tag):
    ...
    summary_path = os.path.join(output_dir, f"checkpoint_pure_noise_global_{year}_{sample_tag}_summary.csv")
    detail_path = os.path.join(output_dir, f"checkpoint_pure_noise_global_{year}_{sample_tag}_detail.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Compare global flow_finalv1_global checkpoints under pure random noise.")
    ...
    print("\n" + "=" * 96)
    print("Global flow_finalv1_global checkpoint sweep: pure random noise only")
