#!/usr/bin/env python3
"""
South Asia v8 checkpoint sweep under pure random noise.
...
from compare_noise_multiv8_sa import (
    build_condition,
    decode_multi,
...
def save_results(results, metadata, output_dir, year, sample_tag):
    ...
    summary_path = os.path.join(output_dir, f"checkpoint_pure_noise_v8_sa_{year}_{sample_tag}_summary.csv")
    detail_path = os.path.join(output_dir, f"checkpoint_pure_noise_v8_sa_{year}_{sample_tag}_detail.csv")
...
def main():
    parser = argparse.ArgumentParser(description="Compare SA v8 checkpoints under pure random noise.")
    ...
    print("\n" + "=" * 96)
    print("South Asia v8 checkpoint sweep: pure random noise only")
