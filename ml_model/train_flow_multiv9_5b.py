...
from flow_matching_multi_v9_5b import FlowMatchingModel, CustomFlowMatcher
from static_geography_v9_5b import STATIC_CHANNEL_NAMES, load_or_build_static_geography
import noise_utils
...
def geos_condition_channel_count(
    ...
    geos_lead_count=4,
    include_temporal_trajectory=True,
):
    """Return v9.5a GEOS channels plus the v9.5b four-lead trajectory block."""
    lead_count = 1 if lead_selective_geos else int(geos_lead_count)
    channels_per_lead = int(geos_statistic_count) * int(geos_variable_count)
    direct_channels = channels_per_lead * lead_count
    trajectory_channels = (
        channels_per_lead * int(geos_lead_count)
        if include_temporal_trajectory
        else 0
    )
    return direct_channels + trajectory_channels
...
def select_geos_condition(x_geos, lead_idx, lead_selective_geos=True):
    ...
    The first block is exactly the v9.5a direct condition (normally the
    requested target lead). The second block always contains all four leads
    and is consumed only by the learned v9.5b temporal-evolution encoder.
    """
    b, statistics, variables, _, h, w = x_geos.shape
    trajectory = x_geos.contiguous().view(b, statistics * variables * 4, h, w)

    if not lead_selective_geos:
        return torch.cat([trajectory, trajectory], dim=1)
    ...
    return torch.cat([target_lead, trajectory], dim=1)
...
def run_val_inference(epoch, model, val_loader, flow_matcher, device, accelerator, output_dir, log_file,
    ...
    for b_idx, batch in enumerate(val_loader):
        ...
        # v9.5b condition: v9.5a target-lead channels + all-lead trajectory.
...
def train(args, accelerator):
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_5b")
    ...
    # ``var_forced`` is the canonical v9.5b switch for entering variance-only
    ...
    use_geos_temporal_encoder = bool(config.get("use_geos_temporal_encoder", True))
    geos_temporal_feature_dim = int(config.get("geos_temporal_feature_dim", 32))
    geos_temporal_heads = int(config.get("geos_temporal_heads", 4))
    geos_temporal_layers = int(config.get("geos_temporal_layers", 2))
    geos_temporal_downsample = int(config.get("geos_temporal_downsample", 4))
    ...
    if not use_geos_temporal_encoder:
        raise ValueError(
            "v9.5b requires use_geos_temporal_encoder=True. "
            "Use v9.5a for the no-temporal-encoder baseline."
        )
    if (
        geos_temporal_feature_dim < 1
        or geos_temporal_heads < 1
        or geos_temporal_layers < 1
        or geos_temporal_downsample < 1
    ):
        raise ValueError("All GEOS temporal encoder dimensions must be >= 1.")
    if geos_temporal_feature_dim % geos_temporal_heads != 0:
        raise ValueError(
            "geos_temporal_feature_dim must be divisible by geos_temporal_heads."
        )
    ...
    if configured_geos_statistics != GEOS_CONDITION_STATISTICS:
        raise ValueError(
            "v9.5b requires geos_condition_statistics="
    ...
    if not np.isclose(
        ...
        raise ValueError(
            "v9.5b dataset and config disagree on geos_member_count_reference: "
    ...
    if accelerator.is_main_process:
        ...
        print(
            f"   [GEOS Temporal]      : all 4 leads -> dim={geos_temporal_feature_dim}, "
            f"heads={geos_temporal_heads}, layers={geos_temporal_layers}, "
            f"spatial downsample={geos_temporal_downsample}"
        )
        ...
        if bool(config.get("plot_topography", True)):
            print(
                f"   [Topo Diagnostic]   : "
                f"{os.path.join(output_dir, 'topography_v9_5b_diagnostic.png')}"
            )
        print(
    ...
    model = FlowMatchingModel(
        ...
        use_geos_temporal_encoder=use_geos_temporal_encoder,
        geos_temporal_feature_dim=geos_temporal_feature_dim,
        geos_temporal_heads=geos_temporal_heads,
        geos_temporal_layers=geos_temporal_layers,
        geos_temporal_downsample=geos_temporal_downsample,
    ).to(device)
    if accelerator.is_main_process:
        ...
        if model.geos_temporal_encoder is not None:
            with torch.no_grad():
                trajectory_probe = torch.zeros(
                    2,
                    len(GEOS_CONDITION_STATISTICS) * 2 * 4,
                    grid_h,
                    grid_w,
                    device=device,
                    dtype=next(model.parameters()).dtype,
                )
                temporal_probe, temporal_summary_probe = model.geos_temporal_encoder(
                    trajectory_probe,
                    lead_idx=torch.tensor([0, 3], device=device),
                )
            expected_temporal_shape = (
                2,
                geos_temporal_feature_dim,
                grid_h,
                grid_w,
            )
            if tuple(temporal_probe.shape) != expected_temporal_shape:
                raise RuntimeError(
                    "GEOS temporal encoder preflight shape mismatch: "
                    f"actual={tuple(temporal_probe.shape)}, "
                    f"expected={expected_temporal_shape}"
                )
            print(
                f"   [Temporal Probe]    : map={tuple(temporal_probe.shape)}, "
                f"summary={tuple(temporal_summary_probe.shape)}"
            )
            del trajectory_probe, temporal_probe, temporal_summary_probe
    ...
    if accelerator.is_main_process:
        ...
        print(
            f"   Raw Condition Input: {model_in_channels + len(STATIC_CHANNEL_NAMES)} "
            f"({model_in_channels} dynamic + {len(STATIC_CHANNEL_NAMES)} static)"
        )
        print(
            f"   UNet Input Channels: "
            f"{accelerator.unwrap_model(model).unet.config.in_channels} "
            f"after replacing the raw 40-channel "
            f"trajectory with {geos_temporal_feature_dim} learned channels"
        )
        ...
        if lead_selective_geos:
            ...
            for statistic in GEOS_CONDITION_STATISTICS:
                ...
                offset += 4
        print(
            f"     [{offset:02d}-{offset + 39:02d}] x_geos: "
            "v9.5b raw four-lead trajectory "
            "(5 statistics x PR/T2M x 4 leads; encoded before UNet)"
        )
        offset += 40
        ...
        print(f"   --- v9.5b Conditioning Upgrades ---")
        print(
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_5b")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_v9_5b.csv")
    component_log_file = os.path.join(output_dir, "training_components_v9_5b.csv")
    metrics_log_file = os.path.join(output_dir, "validation_metrics_v9_5b.csv")
    ...
    if accelerator.is_main_process:
        ...
        # 1. Reverse-normalize the v9.5b GEOS ensemble-mean PR channel.
...
def main():
    ...
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv9_5b.yaml")
