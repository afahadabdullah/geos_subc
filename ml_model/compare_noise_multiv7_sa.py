#!/usr/bin/env python3
"""
South Asia noise-strategy comparison for the multi-target v7 flow model.
...
loads the v7 local/global model class, and writes side-by-side GEOS / random / EOF-LHS
metrics to CSV.
"""
...
from flow_matching_multi_v7 import CustomFlowMatcher, FlowMatchingModel
from train_flow_multiv7 import (
    compute_crps,
    compute_rmse,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare SA multi-v7 local/global noise strategies.")
...
def validate_current_sa_v7_config(config, output_dir):
...
def resolve_t2m_residual_bounds(config):
    ...
    if t2m_target_mode == "geos_residual":
        stats_path = resolve_stats_path(config.get("stats_file", "v7_sa_global_local_stats.pt"))
...
def main():
    ...
    validate_current_sa_v7_config(config, output_dir)
