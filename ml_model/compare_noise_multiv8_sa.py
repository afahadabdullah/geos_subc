#!/usr/bin/env python3
"""
South Asia noise-strategy comparison for the multi-target v8 flow model.
...
loads the v8 local/global model class, and writes side-by-side GEOS / random / EOF-LHS
metrics to CSV.
"""
...
from flow_matching_multi_v8 import CustomFlowMatcher, FlowMatchingModel
from train_flow_multiv8 import (
    compute_crps,
    compute_rmse,
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare SA multi-v8 local/global noise strategies.")
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv8.yaml")
...
def validate_current_sa_v8_config(config, output_dir):
...
def main():
    ...
    validate_current_sa_v8_config(config, output_dir)
