#!/usr/bin/env python3
"""Global noise-strategy comparison for the multi-target flow_finalv1_global model."""
...
from dataset_flow_finalv1_global import GEOS_CONDITION_STATISTICS, S2SHybridDataset
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare global flow_finalv1_global noise strategies.")
...
def validate_global_config(config, output_dir):
    ...
    if target_domain != "global":
        raise ValueError(f"Expected target_domain=global, got {target_domain!r}")
    if bounds:
        raise ValueError(f"Global release must not define target_domain_bounds, got {bounds!r}")
    if output_name != EXPECTED_OUTPUT_DIR:
        raise ValueError(f"Expected output_dir ending in {EXPECTED_OUTPUT_DIR}, got {output_dir!r}")
    if zero_geos:
        raise ValueError("flow_finalv1_global is GEOS-conditioned; zero_geos_condition must be False")
...
def resolve_t2m_residual_bounds(config):
    ...
    if t2m_target_mode == "geos_residual":
        stats_path = resolve_stats_path(config.get("stats_file", "flow_finalv1_global_stats.pt"))
...
def load_noise_context(config, domain_info):
    ...
    if os.path.exists(nao_path):
        context["nao_lookup"] = noise_utils_finalv1_global.parse_nao_index(nao_path)
    if os.path.exists(oni_path):
        context["oni_lookup"] = noise_utils_finalv1_global.parse_oni_index(oni_path)
...
def main():
    ...
    validate_global_config(config, output_dir)
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc="Global noise compare")):
        ...
        for setting in settings:
            ...
            noise = noise_utils_multi_finalv1_global.mix_noise_with_random_multi(
                eof_noise,
                setting["rho_pr"],
                setting["rho_t2m"],
            )
    ...
    csv_path = os.path.join(output_dir, f"noise_comparison_global_{args.year}_{sample_tag}_{ckpt_label}.csv")
