#!/bin/bash
#SBATCH -J SA_ngraw_gen
#SBATCH -o ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m/generate_testmode_%j.log
#SBATCH -e ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m/generate_testmode_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia v9 no-GEOS raw-T2M test-mode-equivalent Zarr generation started at $(date) on $(hostname)"
...
CONFIG_PATH="${SA_TESTGEN_CONFIG:-ml_model/config_flow_multiv9_nogeos_rawt2m.yaml}"
...
OUT_DIR="${SA_TESTGEN_OUT_DIR:-dataprocess/gen_multiv9_sa_55e100e_0n40n_junjul_testmode_nogeos_rawt2m_e100_s50}"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m" ]; then
...
if [ "$CONFIG_T2M_MODE" != "absolute" ]; then
    echo "❌ Refusing raw-T2M generation without absolute T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
if [ "$CONFIG_ZERO_GEOS" != "True" ]; then
    echo "❌ Refusing no-GEOS generation without zero_geos_condition=True"
    exit 1
fi
...
echo "🏁 South Asia v9 no-GEOS raw-T2M test-mode-equivalent Zarr generation finished at $(date)"
