...
Usage:
  python compare_noise_multi_v2.py --output_dir ml_output_flowmulti_v2 --year 2021
"""
...
from train_flow_multiv2 import compute_crps, compute_rmse
...
def run_strategy(model, flow_matcher, batch, device, num_ensemble, num_steps, noise_fn,
    ...
    # Build conditioning for v2 (no GEOS input)
...
def main():
    parser = argparse.ArgumentParser(description="Multi-Variate Noise Comparison (multi v2)")
    parser.add_argument("--output_dir", type=str, default="ml_output_flowmulti_v2")
    ...
    # ─── Model Loading (33-in, 2-out for multi v2) ───
    ...
    def noise_multimodal_dynamic_lhs_val_replay(vB, E, H, W, b, d):
        """
        Match run_val_inference in train_flow_multiv2.py:
    ...
    # Format: (Name, noise_fn, perturb_cond)
    strategies = [
        ("1. Pure Noise", noise_pure, False),
        (
            "3. EOF LHS PR rho0.15 / T2M rho0.05",
    ...
    for b_idx, batch in enumerate(test_loader):
        ...
        for name, fn, perturb_cond in tqdm(strategies, desc=batch_desc, leave=False, ncols=100):
            res = run_strategy(
                model,
                flow_matcher,
                batch,
        ...
        # Total row
        all_crps_out = [geos_out] + [results[name][-1] for name, _, _ in strategies]
