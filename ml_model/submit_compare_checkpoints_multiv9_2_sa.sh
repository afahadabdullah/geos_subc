#!/bin/bash
#SBATCH -J SA_ckpt_v9_2
#SBATCH -o ml_output_checkpoint_compare_sa_v9_2/checkpoint_compare_%j.log
#SBATCH -e ml_output_checkpoint_compare_sa_v9_2/checkpoint_compare_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia v9.2 checkpoint comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_checkpoint_compare_sa_v9_2

CONFIG_PATH="${SA_CKPT_CONFIG:-ml_model/config_flow_multiv9_2.yaml}"
...
python ml_model/compare_checkpoints_multiv9_2_sa.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
    --epochs "$EPOCHS" \
...
echo "🏁 South Asia v9.2 checkpoint comparison finished at $(date)"
