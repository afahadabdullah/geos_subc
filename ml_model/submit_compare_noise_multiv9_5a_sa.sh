#!/bin/bash
#SBATCH -J SA_noise_v9_5a
#SBATCH -o ml_output_noise_compare_sa_v9_5a/compare_noise_%j.log
#SBATCH -e ml_output_noise_compare_sa_v9_5a/compare_noise_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia v9.5a noise comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_noise_compare_sa_v9_5a

CONFIG_PATH="${SA_NOISE_CONFIG:-ml_model/config_flow_multiv9_5a.yaml}"
...
python ml_model/compare_noise_multiv9_5a_sa.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
...
echo "🏁 South Asia v9.5a noise comparison finished at $(date)"
