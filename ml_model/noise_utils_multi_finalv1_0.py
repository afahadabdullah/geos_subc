...
def _resolve_sample_init_date(batch, b_idx, fallback_year):
    ...
    return noise_utils_finalv1_0.datetime.date(int(year), int(month), int(day))
...
def _generate_single_channel_dynamic_noise(
    ...
    if mjo_bases is not None:
        ...
        for b_idx in range(vB):
            ...
            if not use_lhs:
                for j in range(E):
                    mjo_noise[b_idx * E + j, 0] = noise_utils_finalv1_0.sample_from_eof_basis(
                        mjo_bases, phase, lead_idx, device, H, W
                    )
            else:
                mjo_noise[b_idx * E:(b_idx + 1) * E, 0] = noise_utils_finalv1_0.sample_batch_lhs(
    ...
    if nao_bases is not None and nao_lookup is not None:
        ...
        for b_idx in range(vB):
            ...
            nao_phase = noise_utils_finalv1_0.get_nao_phase(init_date, nao_lookup)

            if not use_lhs:
                for j in range(E):
                    nao_noise[b_idx * E + j, 0] = noise_utils_finalv1_0.sample_from_eof_basis(
                        nao_bases, nao_phase, lead_idx, device, H, W
                    )
            else:
                nao_noise[b_idx * E:(b_idx + 1) * E, 0] = noise_utils_finalv1_0.sample_batch_lhs(
    ...
    if enso_bases is not None and oni_lookup is not None:
        ...
        for b_idx in range(vB):
            init_date = init_dates[b_idx]
            lead_idx = int(_get_batch_scalar(leads, b_idx))
            enso_state = noise_utils_finalv1_0.get_enso_state(init_date.month, init_date.year, oni_lookup)

            if not use_lhs:
                for j in range(E):
                    enso_noise[b_idx * E + j, 0] = noise_utils_finalv1_0.sample_from_eof_basis(
                        enso_bases, enso_state, lead_idx, device, H, W
                    )
            else:
                enso_noise[b_idx * E:(b_idx + 1) * E, 0] = noise_utils_finalv1_0.sample_batch_lhs(
    ...
    for b_idx, init_date in enumerate(init_dates):
        ...
        if mjo_df is not None and date_str in mjo_df.index:
            ...
            if not (noise_utils_finalv1_0.pd.isna(r1) or noise_utils_finalv1_0.pd.isna(r2)):
                mjo_amp = float(noise_utils_finalv1_0.np.sqrt(r1**2 + r2**2))

        nao_amp = 0.5
        if nao_lookup is not None:
            nao_amp = abs(noise_utils_finalv1_0.get_nao_value(init_date, nao_lookup))

        enso_amp = 0.5
        if oni_lookup is not None:
            enso_amp = abs(noise_utils_finalv1_0.get_enso_value(init_date.month, init_date.year, oni_lookup))
...
def generate_dynamic_multimodal_noise_multi(
    ...
    if use_lhs and orthogonalize_lhs:
        pr_noise = noise_utils_finalv1_0.orthogonalize_noise_batch(pr_noise, vB, E)
        t2m_noise = noise_utils_finalv1_0.orthogonalize_noise_batch(t2m_noise, vB, E)
...
def mix_noise_with_random_multi(eof_noise, rho_pr, rho_t2m=None):
    ...
    mixed_noise[:, 0:1] = (
        noise_utils_finalv1_0.np.sqrt(max(0.0, 1.0 - rho_pr ** 2)) * rand_noise[:, 0:1]
        + rho_pr * eof_noise[:, 0:1]
    )
    mixed_noise[:, 1:2] = (
        noise_utils_finalv1_0.np.sqrt(max(0.0, 1.0 - rho_t2m ** 2)) * rand_noise[:, 1:2]
