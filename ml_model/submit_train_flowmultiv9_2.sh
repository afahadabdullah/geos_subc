#!/bin/bash
#SBATCH -J SA_flow_v9_2
#SBATCH -o ml_output_flowmulti_v9_2_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v9_2_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v9.2 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv9_2.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv9_2.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv9_2.yaml"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_2_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_flowmultiv9_2.sh"
    else
...
echo "🏁 South Asia Multi-v9.2 local/global residual-T2M training job finished at $(date)"
