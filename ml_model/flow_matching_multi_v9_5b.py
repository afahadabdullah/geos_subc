...
class GeosTemporalEvolutionEncoder(nn.Module):
    """
    Encode the four-lead GEOS trajectory at a coarse spatial resolution.

    Each coarse grid cell supplies four temporal tokens. Every token contains
    the v9.5a mean/std/q10/q90/member-count features for PR and T2M. Attention
    is applied only across the four forecast leads, then the token matching the
    requested target week is returned as a spatial feature map and FiLM summary.
    """
    ...
    def __init__(
        ...
        if feature_dim < 1 or num_heads < 1 or num_layers < 1:
            raise ValueError("GEOS temporal feature_dim, heads, and layers must be >= 1.")
        if feature_dim % num_heads != 0:
            raise ValueError(
                f"GEOS temporal feature_dim={feature_dim} must be divisible by heads={num_heads}."
            )

        self.feature_dim = feature_dim
        self.downsample_factor = max(1, int(downsample_factor))
        per_lead_channels = GEOS_STATISTIC_COUNT * GEOS_VARIABLE_COUNT
        self.spatial_projection = nn.Sequential(
            nn.Conv2d(per_lead_channels, feature_dim, kernel_size=3, padding=1),
            nn.GroupNorm(_num_groups(feature_dim), feature_dim),
            nn.SiLU(),
        )
        self.lead_position = nn.Parameter(
            torch.zeros(1, GEOS_LEAD_COUNT, feature_dim)
        )
    ...
    def forward(self, trajectory, lead_idx=None):
        expected_channels = (
            GEOS_STATISTIC_COUNT * GEOS_VARIABLE_COUNT * GEOS_LEAD_COUNT
        )
        if trajectory.ndim != 4 or trajectory.shape[1] != expected_channels:
            raise ValueError(
                "GEOS trajectory must be "
                f"[B,{expected_channels},H,W], got {tuple(trajectory.shape)}."
            )

        batch, _, height, width = trajectory.shape
        per_lead = (
            trajectory.reshape(
                batch,
                GEOS_STATISTIC_COUNT,
                GEOS_VARIABLE_COUNT,
                GEOS_LEAD_COUNT,
                height,
                width,
            ...
            .permute(0, 3, 1, 2, 4, 5)
            .reshape(
                batch * GEOS_LEAD_COUNT,
                GEOS_STATISTIC_COUNT * GEOS_VARIABLE_COUNT,
                height,
                width,
            )
        )
        features = self.spatial_projection(per_lead)
        ...
        coarse_h, coarse_w = features.shape[-2:]
        features = features.reshape(
            batch, GEOS_LEAD_COUNT, self.feature_dim, coarse_h, coarse_w
        )
        tokens = (
            features.permute(0, 3, 4, 1, 2)
            .reshape(batch * coarse_h * coarse_w, GEOS_LEAD_COUNT, self.feature_dim)
        )
        tokens = self.temporal_attention(
            tokens + self.lead_position.to(dtype=tokens.dtype)
        )

        if lead_idx is None:
            target_lead = torch.zeros(batch, device=trajectory.device, dtype=torch.long)
        else:
            target_lead = lead_idx.to(device=trajectory.device, dtype=torch.long)
            if target_lead.shape != (batch,):
                raise ValueError(
                    f"lead_idx must have shape ({batch},), got {tuple(target_lead.shape)}."
                )
        target_lead = (
            target_lead.view(batch, 1)
            .expand(batch, coarse_h * coarse_w)
            .reshape(-1)
        )
        selected = tokens[
        ...
        selected = (
            selected.reshape(batch, coarse_h, coarse_w, self.feature_dim)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
        selected = self.output_projection(selected)
        ...
        temporal_summary = self.summary_projection(selected.mean(dim=(2, 3)))
        return temporal_map, temporal_summary
...
class FlowMatchingModel(nn.Module):
    ...
    def __init__(
        ...
        use_geos_temporal_encoder=True,
        geos_temporal_feature_dim=32,
        geos_temporal_heads=4,
        geos_temporal_layers=2,
        geos_temporal_downsample=4,
    ):
        super().__init__()
        ...
        if int(out_channels) != 2:
            raise ValueError("v9.5b separate PR/T2M heads require out_channels=2.")
        ...
        self.use_geos_temporal_encoder = bool(use_geos_temporal_encoder)
        if not self.use_geos_temporal_encoder:
            raise ValueError(
                "v9.5b requires the GEOS temporal-evolution encoder. "
                "Use v9.5a for the non-temporal baseline."
            )
        self.geos_trajectory_channels = (
            GEOS_STATISTIC_COUNT * GEOS_VARIABLE_COUNT * GEOS_LEAD_COUNT
            if self.use_geos_temporal_encoder
            else 0
        )
        self.temporal_metadata_channels = 3
        self.geos_temporal_feature_dim = (
            int(geos_temporal_feature_dim)
            if self.use_geos_temporal_encoder
            else 0
        )
        ...
        self.effective_cond_channels = (
            self.cond_channels
            - self.geos_trajectory_channels
            + self.geos_temporal_feature_dim
        )
        if self.effective_cond_channels <= 0:
            raise ValueError(
                f"Invalid effective conditioning width {self.effective_cond_channels}."
            )
        if (
            self.use_geos_temporal_encoder
            and self.cond_channels
            < self.geos_trajectory_channels + self.temporal_metadata_channels
        ):
            raise ValueError(
                "Input conditioning does not contain the expected 40-channel "
                "GEOS trajectory block followed by month/lead metadata."
            )
        backbone_in_channels = (
            out_channels + self.effective_cond_channels + self.static_geography_channels
        )
        ...
        context_input_dim = self.effective_cond_channels * 2
        ...
        self.geos_temporal_encoder = (
            GeosTemporalEvolutionEncoder(
                feature_dim=self.geos_temporal_feature_dim,
                num_heads=geos_temporal_heads,
                num_layers=geos_temporal_layers,
                downsample_factor=geos_temporal_downsample,
                context_dim=CONTEXT_HIDDEN,
            )
            if self.use_geos_temporal_encoder
            else None
    ...
    def _encode_geos_temporal_condition(self, x_cond, lead_idx=None):
        if self.geos_temporal_encoder is None:
            return x_cond, None
        if x_cond.shape[1] != self.cond_channels:
            raise ValueError(
                f"Expected {self.cond_channels} raw conditioning channels, "
                f"got {x_cond.shape[1]}."
            )

        trajectory_end = x_cond.shape[1] - self.temporal_metadata_channels
        trajectory_start = trajectory_end - self.geos_trajectory_channels
        direct_condition = x_cond[:, :trajectory_start]
        trajectory = x_cond[:, trajectory_start:trajectory_end]
        metadata = x_cond[:, trajectory_end:]
        temporal_map, temporal_summary = self.geos_temporal_encoder(
            trajectory,
            lead_idx=lead_idx,
        )
        return (
            torch.cat([direct_condition, temporal_map, metadata], dim=1),
            temporal_summary,
        )

    def prepare_condition(self, x_cond, lead_idx=None):
        """Precompute the static v9.5b temporal condition for repeated ODE calls."""
        return self._encode_geos_temporal_condition(x_cond, lead_idx=lead_idx)
    ...
    def _build_context(
        ...
        if temporal_summary is not None:
            context = context + temporal_summary
    ...
    def forward(
        ...
        if prepared_condition is None:
            x_cond, temporal_summary = self.prepare_condition(
                x_cond,
                lead_idx=lead_idx,
            )
        else:
            x_cond, temporal_summary = prepared_condition
            if x_cond.shape[1] != self.effective_cond_channels:
                raise ValueError(
                    "Prepared conditioning width mismatch: "
                    f"actual={x_cond.shape[1]}, "
                    f"expected={self.effective_cond_channels}."
                )
...
class CustomFlowMatcher:
    ...
    def euler_solve(self, model, noise, x_cond, num_steps=10, lead_idx=None, apply_flow_variance=False,
        ...
        """
        prepared_condition = (
            model.prepare_condition(x_cond, lead_idx=lead_idx)
