...
from flow_matching_multi_v2 import FlowMatchingModel, CustomFlowMatcher
import noise_utils
...
def save_val_plot(epoch, full_pred, true_target_precip, model_crps, model_rmse, geos_pred, geos_crps, geos_rmse, output_dir, 
    ...
    for l in range(4):
        ...
        if l == 0: axes[l, 6].set_title("Model Ens Spread")
    ...
    if has_t2m:
        for l in range(4):
            ...
            if l == 0: axes[row, 6].set_title("T2M Ens Spread")
...
def run_val_inference(epoch, model, val_loader, flow_matcher, device, accelerator, output_dir, log_file, 
    ...
    for b_idx, batch in enumerate(val_loader):
        ...
        # [vB, 31, H, W] = obs(28) + month(2) + lead(1)
        fx_cond = torch.cat([fx_obs, fsin_month, fcos_month, f_lead_channel], dim=1)
        ...
        if print_validation_noise_diag and not did_print_noise_diag and accelerator.is_main_process:
            if use_eof_lhs_noise:
                mode_label = f"PR EOF-LHS rho={validation_rho_pr:.2f} / T2M EOF-LHS rho={validation_rho_t2m:.2f}"
            else:
...
def train(args, accelerator):
    ...
    # ─── Land-Ocean Mask (65% land / 35% ocean) ───
    ...
    mask_cache_path = os.path.join(os.path.dirname(__file__), "land_ocean_mask_v2.pt")
    ...
    if os.path.exists(mask_cache_path):
        ...
        if accelerator.is_main_process:
            print(f"  ✅ V2 Land-Ocean Mask loaded from cache: {mask_cache_path}")
            ...
            try:
                ...
                if n_land > 0 and n_ocean > 0:
                    ...
                    if accelerator.is_main_process:
                        ...
                        print(f"  ✅ V2 Land-Ocean Mask created from {sss_sample_path}")
                        ...
                        plt.suptitle("V2 Land-Ocean Loss Weighting Diagnostic", fontsize=16, fontweight='bold')
                        plt.tight_layout()
                        diag_dir = config.get("output_dir", "ml_output_flowmulti_v2")
    ...
    if not os.path.exists(stats_file):
        raise FileNotFoundError(f"CRITICAL: {stats_file} missing. Please run calculate_global_stats_multi_v2.py first!")
    ...
    # ---------------------------------------------------------
    model = FlowMatchingModel(in_channels=33, out_channels=2).to(device)
    ...
    if accelerator.is_main_process:
        ...
        print(f"   Model Base: FlowMatchingModel V2 (UNet2D Structure)")
        print(f"   Total Input Channels: 33")
        print(f"   --- Condition Channels (x_cond = 31) ---")
        ...
        print(f"     [    28] Month: Sine Temporal Embedding")
        print(f"     [    29] Month: Cosine Temporal Embedding")
        print(f"     [    30] Target Lead: Relative Index Tracking [-1 to +1]")
        print(f"   --- Dynamic Flow Channel (x_t = 2) ---")
        print(f"     [ 31,32] x_t: Pure Noise Vector (Solver Substrate) PR & T2M")
        ...
        print(f"     No variance heads in v2")
        print(f"     Shared UNet features: 64 intermediate channels")
        print(f"-------------------------------------\n")


    # Output directory
    output_dir = config.get("output_dir", "ml_output_flowmulti_v2")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_v2.csv")
    
    if accelerator.is_main_process and not os.path.exists(log_file):
        with open(log_file, "w") as f:
            writer = csv.writer(f)
            writer.writerow(["Epoch", "Train_Loss", "Val_Metric", "Val_PR_CRPS"])
    ...
    if accelerator.is_main_process and eof_bases is not None:
        print("✅ Loaded Multi-Modal EOF bases & Teleconnection Indices for optional structured-noise validation.")
    ...
    crps_phase_reset = False  # Will be set True after velocity-MSE -> CRPS transition.
    top_models = []
            ...
            if accelerator.is_main_process:
                ...
                if not args.test:
                    ...
                    if start_epoch > 101:
                        ...
                        print(f"   📋 Resuming in velocity-MSE phase (<=100). Will reset best_val_loss later.")
                    print()
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, batch in enumerate(pbar):    
            ...
            x_cond = torch.cat([x_obs, sin_month, cos_month, lead_channel], dim=1)
        ...
        if use_crps_phase and not crps_phase_reset:
            if accelerator.is_main_process:
                ...
                print(f"   Resetting best_val_loss from {best_val_loss:.4f} (Velocity MSE) → inf (CRPS)")
            best_val_loss = float('inf')
            crps_phase_reset = True
        ...
        if accelerator.is_main_process:
            if use_crps_phase:
                ...
                print(f"\n⌛ Epoch {epoch} complete. Starting Fast Validation (Velocity MSE)...")
        ...
        # ============================================================
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    loss_val = (spatial_weights * temp_weights * (v_pred - v_target)**2).mean()
            ...
            if accelerator.is_main_process:
                print(f"✅ Velocity MSE Validation Complete. Avg Loss: {current_val_metric:.4f}")

                is_new_best = (current_val_metric < best_val_loss)
                
                if is_new_best:
                    print(f"🏆 NEW BEST (Velocity MSE)! {current_val_metric:.4f} (Prev: {best_val_loss:.4f})")
                    best_val_loss = current_val_metric

                    new_best_name = f"best_model_epoch_{epoch}_vel_mse_{current_val_metric:.4f}.pt"
                    new_best_path = os.path.join(output_dir, new_best_name)
                    unwrapped_model = accelerator.unwrap_model(model)
                    best_ckpt = {
                        'epoch': epoch,
                    ...
                    registry_path = os.path.join(output_dir, "model_registry.json")
                    registry = json.load(open(registry_path)) if os.path.exists(registry_path) else []
                    registry.append({"rank": 0, "path": new_best_path, "val_loss": current_val_metric, "epoch": epoch, "metric": "velocity_mse"})
                    registry.sort(key=lambda x: x['val_loss'])
                    for i, entry in enumerate(registry): entry['rank'] = i + 1
                    with open(registry_path, 'w') as f: json.dump(registry, f, indent=2)
                    print(f"📋 Registry: {len(registry)} models tracked.")
                ...
                with open(log_file, "a") as f:
                    csv.writer(f).writerow([epoch, avg_train_loss, current_val_metric, 0.0])
...
def main():
    ...
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv2.yaml")
