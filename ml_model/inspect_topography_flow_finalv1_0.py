#!/usr/bin/env python3
"""Preflight the GLDAS elevation file before launching expensive flow_finalv1.0 training."""
...
from static_geography_flow_finalv1_0 import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inspect GLDAS topography schema and verify the flow_finalv1.0 South Asia crop."
    )
    ...
    parser.add_argument(
        "--output",
        default="ml_output_flow_finalv1_0_sa_55e100e_0n40n_noisectx_t2mres/"
        "topography_flow_finalv1_0_preflight.png",
...
def main():
    ...
    print("=" * 88)
    print("flow_finalv1.0 GLDAS TOPOGRAPHY PREFLIGHT")
    print(f"File: {path}")
    ...
    axes[0].set_title("GLDAS elevation on flow_finalv1.0 grid (m)")
    ...
    fig.suptitle(
        f"flow_finalv1.0 topography preflight | {os.path.basename(path)}\n"
    ...
    plt.close(fig)
    print(f"\nPASS: topography is compatible with flow_finalv1.0")
