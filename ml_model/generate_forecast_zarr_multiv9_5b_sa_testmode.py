#!/usr/bin/env python3
"""
Generate South Asia multi-v9.5b ensemble forecasts exactly like v9 test mode.

Unlike generate_forecast_zarr_multiv9_5b_sa.py, this script creates the full
EOF-LHS/random mixed ensemble noise for each batch before chunking the ODE solve.
That matches train_flow_multiv9_5b.run_full_test_suite_multi and avoids changing
...
from compare_noise_multiv9_5b_sa import (
    decode_multi,
    load_noise_context,
    resolve_checkpoint,
...
from dataset_flow_multi_v9_5b import S2SHybridDataset, resolve_target_domain
from flow_matching_multi_v9_5b import CustomFlowMatcher, FlowMatchingModel
from train_flow_multiv9_5b import (
    euler_solve_chunked,
...
DEFAULT_OUT_DIR = "dataprocess/gen_multiv9_5b_sa_55e100e_0n40n_junjul_testmode_e100_s50"


def parse_args():
    parser = argparse.ArgumentParser(description="Generate multi-v9.5b SA forecasts to yearly Zarr stores.")
    ...
    parser.add_argument("--clim_num_ensemble", type=int, default=100, help="Members/year before --eval_start_year.")
    ...
    parser.add_argument("--clean_out_dir", action="store_true", help="Remove --out_dir before starting.")
...
def build_condition(batch, device, zero_geos_condition=False, lead_selective_geos=True):
...
def generate_batch_forecast(
    ...
    state_chunk_size = int(ode_batch_size) if ode_batch_size is not None and int(ode_batch_size) > 0 else None
    ensemble_chunk = int(num_ensemble)
    if ensemble_chunk_size is not None:
        ensemble_chunk = min(ensemble_chunk, max(1, int(ensemble_chunk_size)))
    if state_chunk_size is not None:
        ensemble_chunk = min(ensemble_chunk, max(1, state_chunk_size // max(1, batch_size)))
    ensemble_chunk = max(1, ensemble_chunk)

    current_year = int(batch["year"][0].item())
    noise_full = generate_noise(
        batch=batch,
    ...
    noise_by_ensemble = noise_full.view(batch_size, num_ensemble, 2, height, width)

    pr_chunks = []
    t2m_chunks = []
    for ens_start in range(0, num_ensemble, ensemble_chunk):
        ens_count = min(ensemble_chunk, num_ensemble - ens_start)
        x_cond_expanded = (
            x_cond.unsqueeze(1)
        ...
        noise = noise_by_ensemble[:, ens_start:ens_start + ens_count].reshape(batch_size * ens_count, 2, height, width)
        ...
        with get_autocast_context(device, mixed_precision):
            pred_norm = euler_solve_chunked(
                ...
                chunk_size=batch_size * ens_count,
...
def write_year(year, args, config, model, flow_matcher, device, checkpoint_path, checkpoint_meta, noise_context, deadline=None):
    ...
    attrs = {
        ...
        "generation_mode": "test_mode_equivalent_full_noise_then_ode_chunks",
        "model_version": "multiv9_5b",
    ...
    seen_inits = 0
    pbar = tqdm(loader, desc=f"Generate v9.5b SA {year}")
...
def main():
    ...
    if args.clean_out_dir and os.path.exists(args.out_dir):
        print(f"♻️ Removing existing output directory before generation: {args.out_dir}")
        remove_path(args.out_dir)
    ...
    print("\n" + "=" * 88)
    print("South Asia v9.5b forecast Zarr generation (test-mode equivalent)")
    ...
    print("  Chunking     : full ensemble noise first, then ODE chunks (matches --test)")
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        if not completed:
            ...
            break
    if all_complete:
        print("✅ Requested v9.5b SA Zarr generation range is complete.")
    else:
        print("⏸️ Requested v9.5b SA Zarr generation range is incomplete and can be resumed.")
