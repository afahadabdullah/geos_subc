#!/usr/bin/env python3
"""
Generate yearly multi-v3 ensemble forecasts and save them as Zarr stores.

Outputs are written under dataprocess/gen_multiv3/ by default and mirror the
...
from train_flow_multiv3 import decode_multi_forecast_raw, euler_solve_chunked


def parse_args():
    parser = argparse.ArgumentParser(description="Generate and save multi-v3 ensemble forecasts to yearly Zarr stores.")
    ...
    parser.add_argument("--out_dir", type=str, default="dataprocess/gen_multiv3", help="Directory for yearly generated Zarr stores.")
...
def write_year(
    ...
    default_use_eof_lhs_noise = bool(config.get("inference_use_eof_lhs_noise", True))
    default_use_flow_variance = bool(config.get("inference_use_flow_variance", False))
    use_flow_variance = (not args.pure_noise) and default_use_flow_variance
    use_eof_lhs_noise = (not args.pure_noise) and default_use_eof_lhs_noise
    ...
    dataset_attrs = {
        ...
        "generation_mode": "pure_noise" if args.pure_noise else "configured_inference_noise",
    ...
    accelerator.print(
        ...
        f"generation_mode={'pure_noise' if args.pure_noise else 'configured_inference_noise'}, "
...
def main():
    ...
    args.output_dir = args.output_dir or config.get("output_dir", "ml_output_flowmulti_v3")
