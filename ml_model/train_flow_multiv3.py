...
def train(args, accelerator):
    ...
    crps_train_max_ensemble_per_chunk = int(config.get("crps_loss_max_ensemble_per_chunk", crps_train_num_ensemble))
    crps_train_pr_weight = float(config.get("crps_loss_pr_weight", 1.0))
    crps_train_t2m_weight = float(config.get("crps_loss_t2m_weight", 1.0))
    crps_train_use_land_ocean_weights = bool(config.get("crps_loss_use_land_ocean_weights", False))
    crps_train_use_gradient_checkpointing = bool(config.get("crps_loss_use_gradient_checkpointing", True))
    ...
    if accelerator.is_main_process:
        ...
        print(f"   [Training Mode]      : {'CRPS' if crps_loss else 'Velocity MSE'}")
        print(
            f"   [Validation Switch]  : Epoch 1-{phase_transition_epoch} = MSE val, "
            f"{phase_transition_epoch + 1}+ = CRPS val"
        )
        ...
        print(f"   [CRPS Ens/Chunk]     : {crps_train_max_ensemble_per_chunk}")
        print(f"   [CRPS Weights]       : PR={crps_train_pr_weight:.2f}, T2M={crps_train_t2m_weight:.2f}")
        print(f"   [CRPS Spatial Wt]    : {train_weight_mode}")
        print(f"   [CRPS Grad Ckpt]     : {crps_train_use_gradient_checkpointing}")
        print(f"   [CRPS Cost Hint]     : ~{rollout_states} states x {crps_train_num_steps} Euler steps per batch")
        ...
        print(f"   [Inference Noise]    : EOF-LHS={inference_use_eof_lhs_noise}, FlowVar={inference_use_flow_variance}")
        print(f"   [Test Noise]         : EOF-LHS={test_use_eof_lhs_noise}, FlowVar={test_use_flow_variance}")
    ...
    top_models = []
    current_phase = "crps" if crps_loss else "mse"
        ...
        try:
            ...
            if not args.test:
                ...
                try:
                    ...
                    if accelerator.is_main_process:
                        print("   ⚠️ Optimizer state mismatch. Starting with fresh optimizer state.")
                        print(f"      Error: {e}")
            ...
            if accelerator.is_main_process:
                ...
                if not args.test:
                    ...
                    print(f"   Loaded Train Mode: {loaded_phase_name}")
                    print(f"   Configured Train Mode: {current_phase}")
                    print(f"   Best Val Loss so far: {best_val_loss:.4f}")
                    if start_epoch > (phase_transition_epoch + 1):
                        print(
                            f"   ✅ Resuming deep in CRPS validation phase (>{phase_transition_epoch + 1}). "
                            "best_val_loss is already CRPS-scale."
                        )
                    elif start_epoch == (phase_transition_epoch + 1):
                        print(
                            f"   🔀 Resuming exactly at the CRPS validation boundary "
                            f"(Epoch {phase_transition_epoch + 1}). Will reset best_val_loss."
                        )
                    else:
                        print(
                            f"   📋 Resuming in MSE validation phase (<= {phase_transition_epoch}). "
    ...
    if args.test:
        ...
        run_full_test_suite_multi(
            ...
            lats=plot_lats,
            lons=plot_lons,
            use_flow_variance=test_use_flow_variance,
            eof_bases=eof_bases,
            nao_bases=nao_bases,
            nao_lookup=nao_lookup,
            ...
            t2m_nao_bases=t2m_nao_bases,
            t2m_enso_bases=t2m_enso_bases,
            use_eof_lhs_noise=test_use_eof_lhs_noise,
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        for i, batch in enumerate(pbar):    
            ...
            if crps_loss:
                ...
                train_spatial_weights = spatial_weights if crps_train_use_land_ocean_weights else area_weights
                loss, crps_diag = compute_multi_crps_training_loss(
                    ...
                    num_ensemble=crps_train_num_ensemble,
                    num_steps=crps_train_num_steps,
                    target_sqrt_min=target_sqrt_min,
                    target_sqrt_max=target_sqrt_max,
                    spatial_weights=train_spatial_weights,
                    ode_chunk_size=crps_train_ode_batch_size,
                    max_ensemble_per_chunk=crps_train_max_ensemble_per_chunk,
                    pr_weight=crps_train_pr_weight,
                    t2m_weight=crps_train_t2m_weight,
                    use_checkpoint=crps_train_use_gradient_checkpointing,
                )
                ...
                # Keep the variance-head forward path active, matching multiv1.
                ...
                # Match multiv1 behavior: the main pretraining phase still optimizes
                # velocity MSE, but the variance head remains in the forward path.
                _ = loss_var
                loss = loss_vel
        ...
        # Phase 2 (epoch > phase_transition_epoch): CRPS-based validation
        #   Every epoch: run_val_inference, periodic ckpt every 5 epochs
        
        use_crps_phase = (epoch > phase_transition_epoch)
        
        # One-time reset: when transitioning from MSE validation to CRPS validation,
        ...
        # ever be saved as "new best" in the CRPS phase.
...
def main():
    parser = argparse.ArgumentParser(description="Train or test the multi-target flow-matching v3 model")
    parser.add_argument("--config", type=str, default="ml_model/config_flow_multiv3.yaml")
