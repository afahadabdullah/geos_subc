#!/bin/bash
#SBATCH -J SA_flow_v7
#SBATCH -o ml_output_flowmulti_v7_south_asia_global_context_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v7_south_asia_global_context_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v7 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv7.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv7.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv7.yaml"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v7_south_asia_global_context_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to train v7 without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
if [ ! -f "$STATS_PATH" ]; then
    echo "📊 Stats file not found. Computing v7 global/local statistics from $DATA_DIR_OVERRIDE..."
    python ml_model/calculate_global_local_stats_multi_v7.py \
        --data_root "$DATA_DIR_OVERRIDE" \
        --out "$STATS_PATH" \
        --target_domain "$CONFIG_TARGET_DOMAIN"
else
    echo "✅ v7 global/local stats found at $STATS_PATH. Skipping recalculation."
fi
...
echo "🔥 Launching Flow Matching Multi-Target v7 Training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_multiv7.py --config "$CONFIG_PATH" \
    --epochs-per-run 20
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_flowmultiv7.sh"
    else
...
echo "🏁 South Asia Multi-v7 local/global residual-T2M training job finished at $(date)"
