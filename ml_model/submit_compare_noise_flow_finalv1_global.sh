#!/bin/bash
#SBATCH -J GLOBAL_noise_finalv1
#SBATCH -o ml_output_noise_compare_global_flow_finalv1/compare_noise_%j.log
#SBATCH -e ml_output_noise_compare_global_flow_finalv1/compare_noise_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 Global flow_finalv1_global noise comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_noise_compare_global_flow_finalv1

CONFIG_PATH="${GLOBAL_NOISE_CONFIG:-ml_model/config_flow_finalv1_global.yaml}"
...
YEAR="${GLOBAL_NOISE_YEAR:-2021}"
CHECKPOINT="${GLOBAL_NOISE_CHECKPOINT:-best_flow_ckpt.pt}"
NUM_ENSEMBLE="${GLOBAL_NOISE_ENSEMBLE:-$CONFIG_TEST_ENSEMBLE}"
NUM_STEPS="${GLOBAL_NOISE_STEPS:-$CONFIG_TEST_STEPS}"
BATCH_LIMIT="${GLOBAL_NOISE_BATCH_LIMIT:-12}"
ODE_BATCH_SIZE="${GLOBAL_NOISE_ODE_BATCH:-120}"
FULL_YEAR="${GLOBAL_NOISE_FULL_YEAR:-0}"
SETTING_ARGS=()
if [ -n "${GLOBAL_NOISE_SETTINGS:-}" ]; then
    IFS=';' read -ra SETTINGS <<< "$GLOBAL_NOISE_SETTINGS"
...
if [ "$CONFIG_TARGET_DOMAIN" != "global" ]; then
    echo "❌ Refusing to run non-global config: target_domain=$CONFIG_TARGET_DOMAIN"
    exit 1
fi
...
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to run v9 compare without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
echo "🎯 Extra settings: ${GLOBAL_NOISE_SETTINGS:-config default}"

python ml_model/compare_noise_flow_finalv1_global.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
...
echo "🏁 Global flow_finalv1_global noise comparison finished at $(date)"
