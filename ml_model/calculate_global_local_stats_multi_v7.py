#!/usr/bin/env python3
"""
Build v7 stats for the South Asia local/global multi-target flow model.

The v7 normalization file intentionally mixes two scopes:
- global predictor bounds for variables used by the full-global context encoder
- South Asia local bounds for the T2M residual target
...
def scan_sa_t2m_residual(bounds, data_root, start_year, end_year, batch_size, target_domain):
    print(f"Scanning {target_domain} T2M residual target bounds ({start_year}-{end_year})...")
    dataset = S2SHybridDataset(
        data_root=data_root,
        start_year=start_year,
        end_year=end_year,
...
def calculate_stats(data_root, out_path, start_year, end_year, batch_size, target_domain):
    bounds = {
    ...
    scan_sa_t2m_residual(bounds, data_root, start_year, end_year, batch_size, target_domain)
    finalize_bounds(bounds)

    print("\n==================================")
    print(f"Calculated v7 Global/Local Bounds ({start_year}-{end_year})")
    print("==================================")
    for key, value in bounds.items():
...
def main():
    ...
    parser.add_argument("--out", type=str, default="ml_model/v7_sa_global_local_stats.pt")
