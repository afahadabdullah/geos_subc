#!/bin/bash
#SBATCH -J SA_ckpt_v9_3
#SBATCH -o ml_output_checkpoint_compare_sa_v9_3/checkpoint_compare_%j.log
#SBATCH -e ml_output_checkpoint_compare_sa_v9_3/checkpoint_compare_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 South Asia v9.3 checkpoint comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_checkpoint_compare_sa_v9_3

CONFIG_PATH="${SA_CKPT_CONFIG:-ml_model/config_flow_multiv9_3.yaml}"
...
python ml_model/compare_checkpoints_multiv9_3_sa.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
    --epochs "$EPOCHS" \
...
echo "🏁 South Asia v9.3 checkpoint comparison finished at $(date)"
