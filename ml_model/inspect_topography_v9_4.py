#!/usr/bin/env python3
"""Preflight the GLDAS elevation file before launching expensive v9.4 training."""
...
from static_geography_v9_4 import (
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Inspect GLDAS topography schema and verify the v9.4 South Asia crop."
    )
    ...
    parser.add_argument(
        "--output",
        default="ml_output_flowmulti_v9_4_sa_55e100e_0n40n_noisectx_t2mres/"
        "topography_v9_4_preflight.png",
...
def main():
    ...
    print("=" * 88)
    print("v9.4 GLDAS TOPOGRAPHY PREFLIGHT")
    print(f"File: {path}")
    ...
    axes[0].set_title("GLDAS elevation on v9.4 grid (m)")
    ...
    fig.suptitle(
        f"v9.4 topography preflight | {os.path.basename(path)}\n"
    ...
    plt.close(fig)
    print(f"\nPASS: topography is compatible with v9.4")
