#!/usr/bin/env python3
"""
Build lead-specific monthly climatology from 1999-2021 for:
- ML multiv1 forecasts (ensemble mean per init date first)
- Raw GEOS forecasts (ensemble mean per init date first)
- Observations

The monthly grouping is by init month, not valid month. That makes this a
forecast-system climatology suitable for lead-aware anomaly baselines.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Build monthly lead-specific climatology for ML, GEOS, and observations.")
...
def init_accumulator(lead_values: np.ndarray, y_values: np.ndarray, x_values: np.ndarray) -> Dict[str, np.ndarray]:
    ...
    return {
        "pr_sum": np.zeros((12, lead_count, y_count, x_count), dtype=np.float64),
        "pr_count": np.zeros((12, lead_count, y_count, x_count), dtype=np.float64),
        "tas_sum": np.zeros((12, lead_count, y_count, x_count), dtype=np.float64),
        "tas_count": np.zeros((12, lead_count, y_count, x_count), dtype=np.float64),
        "n_init_pr": np.zeros(12, dtype=np.int32),
        "n_init_tas": np.zeros(12, dtype=np.int32),
    }


def update_monthly_accumulator(acc: Dict[str, np.ndarray], values: np.ndarray, months: np.ndarray, var_key: str):
    # values shape [S, L, Y, X]
    for month in range(1, 13):
        ...
        acc[f"{var_key}_sum"][month - 1] += np.where(valid, chunk, 0.0).sum(axis=0)
        acc[f"{var_key}_count"][month - 1] += valid.sum(axis=0)
        acc[f"n_init_{var_key}"][month - 1] += int(np.count_nonzero(mask))
...
def standardize_var(ds: xr.Dataset, layout: Dict[str, str], var_name: str, ensemble_mean_first: bool) -> Tuple[np.ndarray, np.ndarray]:
    ...
    months = np.asarray([item.month for item in s_values], dtype=np.int32)
    return np.asarray(da.values, dtype=np.float64), months
...
def write_climatology_store(
    ...
    ds_out = xr.Dataset(
        data_vars={
            "pr": (("month", "L", "Y", "X"), finalize_mean(acc["pr_sum"], acc["pr_count"])),
            "tas": (("month", "L", "Y", "X"), finalize_mean(acc["tas_sum"], acc["tas_count"])),
            "n_init_pr": (("month",), acc["n_init_pr"].astype(np.int32)),
            "n_init_tas": (("month",), acc["n_init_tas"].astype(np.int32)),
        },
        coords={
        ...
        attrs={
            "climatology_kind": "init_month_and_lead",
            "description": "Monthly climatology averaged over init dates in each calendar month; GEOS and ML use ensemble mean per init first.",
    ...
    ds_out["pr"].attrs.update({"long_name": "monthly_init_climatology_pr"})
    ds_out["tas"].attrs.update({"long_name": "monthly_init_climatology_tas"})
    ds_out["month"].attrs.update({"long_name": "initialization_month"})
...
def build_ml_climatology(args, data_dir: str):
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        try:
            ...
            pr_values, months = standardize_var(ds, layout, layout["pr_var"], ensemble_mean_first=True)
            tas_values, months_tas = standardize_var(ds, layout, layout["tas_var"], ensemble_mean_first=True)
            if not np.array_equal(months, months_tas):
                raise ValueError(f"ML month alignment mismatch in {path}")
            update_monthly_accumulator(acc, pr_values, months, "pr")
            update_monthly_accumulator(acc, tas_values, months, "tas")
            used_paths.append(path)
        finally:
            ds.close()

    out_path = os.path.join(args.output_dir, f"ml_monthly_ensmean_clim_{args.start_year}_{args.end_year}.zarr")
...
def build_geos_climatology(args, data_dir: str):
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        try:
            ...
            tas_values, months_tas = standardize_var(ds, layout, layout["tas_var"], ensemble_mean_first=True)
            if not np.array_equal(months, months_tas):
                raise ValueError(f"GEOS month alignment mismatch in {path}")
            update_monthly_accumulator(acc, pr_values, months, "pr")
            update_monthly_accumulator(acc, tas_values, months, "tas")
        finally:
            ds.close()

    out_path = os.path.join(args.output_dir, f"geos_monthly_ensmean_clim_{args.start_year}_{args.end_year}.zarr")
    write_climatology_store(
        out_path=out_path,
        source_name="geos",
        start_year=args.start_year,
...
def build_obs_climatology(args, data_dir: str):
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        try:
            ...
            pr_values, months = standardize_var(ds_pr, layout_pr, layout_pr["pr_var"], ensemble_mean_first=False)
            tas_values, months_tas = standardize_var(ds_tas, layout_tas, layout_tas["tas_var"], ensemble_mean_first=False)
            if not np.array_equal(months, months_tas):
                raise ValueError(f"Obs month alignment mismatch in {year}")
            update_monthly_accumulator(acc, pr_values, months, "pr")
            update_monthly_accumulator(acc, tas_values, months, "tas")
        finally:
            ds_pr.close()
    ...
    out_path = os.path.join(args.output_dir, f"obs_monthly_clim_{args.start_year}_{args.end_year}.zarr")
    write_climatology_store(
        out_path=out_path,
...
def main():
    ...
    print("=" * 80)
    print("MONTHLY MULTIV1 CLIMATOLOGY")
    print(f"Years      : {args.start_year}-{args.end_year}")
    print(f"Data Dir   : {data_dir}")
    print(f"Output Dir : {os.path.abspath(args.output_dir)}")
    print("Basis      : init-month + lead; ML/GEOS use ensemble mean per init first")
    print("=" * 80)
    ...
    print("\n✅ All climatology stores written successfully.")
