#!/bin/bash
#SBATCH -J GLOBAL_ckpt_finalv1
#SBATCH -o ml_output_checkpoint_compare_global_flow_finalv1/checkpoint_compare_%j.log
#SBATCH -e ml_output_checkpoint_compare_global_flow_finalv1/checkpoint_compare_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 Global flow_finalv1_global checkpoint comparison started at $(date) on $(hostname)"
...
mkdir -p ml_output_checkpoint_compare_global_flow_finalv1

CONFIG_PATH="${GLOBAL_CKPT_CONFIG:-ml_model/config_flow_finalv1_global.yaml}"
...
YEAR="${GLOBAL_CKPT_YEAR:-2021}"
EPOCHS="${GLOBAL_CKPT_EPOCHS:-70,80,90,100,110,120,130,140}"
BEST_CHECKPOINT="${GLOBAL_CKPT_BEST_CHECKPOINT:-best_flow_ckpt.pt}"
NUM_ENSEMBLE="${GLOBAL_CKPT_ENSEMBLE:-30}"
NUM_STEPS="${GLOBAL_CKPT_STEPS:-10}"
ODE_BATCH_SIZE="${GLOBAL_CKPT_ODE_BATCH:-120}"
BATCH_LIMIT="${GLOBAL_CKPT_BATCH_LIMIT:-12}"
FULL_YEAR="${GLOBAL_CKPT_FULL_YEAR:-0}"
STRICT="${GLOBAL_CKPT_STRICT:-1}"
SEED="${GLOBAL_CKPT_SEED:-1234}"
...
python ml_model/compare_checkpoints_flow_finalv1_global.py \
    --config "$CONFIG_PATH" \
    --year "$YEAR" \
    --epochs "$EPOCHS" \
...
echo "🏁 Global flow_finalv1_global checkpoint comparison finished at $(date)"
