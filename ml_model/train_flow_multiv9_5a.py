...
# Local Modules
from dataset_flow_multi_v9_5a import (
...
from flow_matching_multi_v9_5a import FlowMatchingModel, CustomFlowMatcher
...
def select_geos_condition(x_geos, lead_idx, lead_selective_geos=True):
    ...
    v9.5a uses only the GEOS forecast lead matching each training sample. Set
...
def train(args, accelerator):
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_5a")
    ...
    # ``var_forced`` is the canonical v9.5a switch for entering variance-only
    ...
    if configured_geos_statistics != GEOS_CONDITION_STATISTICS:
        raise ValueError(
            "v9.5a requires geos_condition_statistics="
    ...
    if not np.isclose(
        ...
        raise ValueError(
            "v9.5a dataset and config disagree on geos_member_count_reference: "
    ...
    if accelerator.is_main_process:
        ...
        if bool(config.get("plot_topography", True)):
            print(
                f"   [Topo Diagnostic]   : "
                f"{os.path.join(output_dir, 'topography_v9_5a_diagnostic.png')}"
            )
        print(
    ...
    if accelerator.is_main_process:
        ...
        print(f"   --- v9.5a Conditioning Upgrades ---")
        print(
    ...
    output_dir = config.get("output_dir", "ml_output_flowmulti_v9_5a")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_v9_5a.csv")
    component_log_file = os.path.join(output_dir, "training_components_v9_5a.csv")
    metrics_log_file = os.path.join(output_dir, "validation_metrics_v9_5a.csv")
    ...
    if accelerator.is_main_process:
        ...
        # 1. Reverse-normalize the v9.5a GEOS ensemble-mean PR channel.
