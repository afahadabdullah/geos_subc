#!/bin/bash
#SBATCH -J SA_test_v8
#SBATCH -o ml_output_flowmulti_v8_sa_55e100e_0n40n_t2mres/test_%j.log
#SBATCH -e ml_output_flowmulti_v8_sa_55e100e_0n40n_t2mres/test_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🧪 South Asia Multi-v8 full test started at $(date) on $(hostname)"
...
CONFIG_PATH="${SA_TEST_CONFIG:-ml_model/config_flow_multiv8.yaml}"
...
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to test v8 without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" \
    ml_model/train_flow_multiv8.py \
    --config "$TEST_CONFIG" \
    --test \
    --ckpt "$CKPT_ARG"

echo "🏁 South Asia Multi-v8 full test finished at $(date)"
