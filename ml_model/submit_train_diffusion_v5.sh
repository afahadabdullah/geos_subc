...
#SBATCH -o ml_output_diffusion_v5/diff_v5_%j.log
#SBATCH -e ml_output_diffusion_v5/diff_v5_%j.log
...
mkdir -p ml_output_diffusion_v5
...
echo "🔥 Launching V5 Diffusion Training [15 Epochs this session]..."
accelerate launch --num_processes 1 --mixed_precision fp16 ml_model/train_diffusion_v5.py --config ml_model/config_diffusion_v5.yaml --epochs-per-run 15
...
CKPT_FILE="ml_output_diffusion_v5/latest_diffusion_ckpt_v5.pt"
MAX_EPOCHS=1000
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$((MAX_EPOCHS - 1))" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_diffusion_v5.sh"
    else
