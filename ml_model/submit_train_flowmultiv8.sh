#!/bin/bash
#SBATCH -J SA_flow_v8
#SBATCH -o ml_output_flowmulti_v8_sa_55e100e_0n40n_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v8_sa_55e100e_0n40n_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v8 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv8.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv8.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv8.yaml"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v8_sa_55e100e_0n40n_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
if [ "$CONFIG_LAT_MIN" != "0.0" ] || [ "$CONFIG_LAT_MAX" != "40.0" ] || [ "$CONFIG_LON_MIN" != "55.0" ] || [ "$CONFIG_LON_MAX" != "100.0" ]; then
    echo "❌ Refusing unexpected v8 target bounds: lat=$CONFIG_LAT_MIN..$CONFIG_LAT_MAX lon=$CONFIG_LON_MIN..$CONFIG_LON_MAX"
    exit 1
fi
if [ "$CONFIG_T2M_MODE" != "geos_residual" ]; then
    echo "❌ Refusing to train v8 without residual T2M mode: t2m_target_mode=$CONFIG_T2M_MODE"
    exit 1
fi
...
echo "🔥 Launching Flow Matching Multi-Target v8 Training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_multiv8.py --config "$CONFIG_PATH" \
    --epochs-per-run 20
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_flowmultiv8.sh"
    else
...
echo "🏁 South Asia Multi-v8 local/global residual-T2M training job finished at $(date)"
