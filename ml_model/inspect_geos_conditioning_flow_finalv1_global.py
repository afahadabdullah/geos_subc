#!/usr/bin/env python3
"""Fail-fast verification of flow_finalv1_global targets and GEOS statistics."""
...
from dataset_flow_stats_finalv1_global import S2SHybridDataset as ReferenceDataset
from dataset_flow_finalv1_global import (
    GEOS_CONDITION_STATISTICS,
    S2SHybridDataset as FinalDataset,
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify flow_finalv1_global GEOS statistics without changing reference targets."
    )
    parser.add_argument("--config", default="ml_model/config_flow_finalv1_global.yaml")
...
def main():
    ...
    reference = make_dataset(ReferenceDataset, config, year)
    current = make_dataset(FinalDataset, config, year)
    if len(reference) != len(current):
        raise AssertionError(
            f"Dataset length changed: reference={len(reference)}, "
            f"flow_finalv1_global={len(current)}"
        )

    sample_index = int(args.sample_index)
    if sample_index < 0 or sample_index >= len(current):
        raise IndexError(f"sample-index {sample_index} outside 0..{len(current) - 1}")
    reference_sample = reference[sample_index]
    final_sample = current[sample_index]
    ...
    for key in (
        ...
        assert_close(key, final_sample[key], reference_sample[key])

    x_geos = final_sample["x_geos"]
    expected_shape = (
    ...
    raw = final_sample["geos_ens_raw"].float()
    member_count = int(raw.shape[0])
    member_count_by_lead = final_sample["geos_member_count_raw"].float()
    ...
    print("PASS: flow_finalv1_global target/predictor equivalence and GEOS statistics")
    print(f"  config       : {args.config}")
    print(f"  year/index   : {year}/{sample_index}")
    print(f"  init/lead    : {final_sample['year']}-{final_sample['month']:02d}-{final_sample['day']:02d} / {int(final_sample['lead_idx']) + 1}")
    print(
    ...
    print("  targets      : exactly match the reference dataset within atol=1e-6")
