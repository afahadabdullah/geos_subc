#!/bin/bash
#SBATCH -J SA_test_ngraw
#SBATCH -o ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m/test_%j.log
#SBATCH -e ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m/test_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🧪 South Asia Multi-v9 no-GEOS raw-T2M full test started at $(date) on $(hostname)"
...
CONFIG_PATH="${SA_TEST_CONFIG:-ml_model/config_flow_multiv9_nogeos_rawt2m.yaml}"
...
EXPECTED_OUTPUT_DIR="ml_output_flowmulti_v9_sa_55e100e_0n40n_noisectx_nogeos_rawt2m"
...
if [ "$CONFIG_ZERO_GEOS" != "True" ]; then
    echo "❌ Refusing no-GEOS test without zero_geos_condition=True"
    exit 1
fi
...
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" \
    ml_model/train_flow_multiv9.py \
    --config "$TEST_CONFIG" \
    --test \
    --ckpt "$CKPT_ARG"

echo "🏁 South Asia Multi-v9 no-GEOS raw-T2M full test finished at $(date)"
