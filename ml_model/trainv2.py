...
# ------------------------------------------------
"""
GEOS S2S3 Bias Correction Training Script (V2: Spherical Enhancements)

This script trains a Conditional UNet with Gaussian Diffusion (DDPM/DDIM) on Residuals.

Key Features (V2):
- Spherical Padding: Wraps longitude (circular) and zero-pads latitude.
- Latitude-Weighted Loss: MSE weighted by cos(lat) to account for polar area distortion.
- Residual Learning: Targets (GPCP - GEOS).
- Conditioning: GEOS Forecast, Observed State, MJO, Month.
...
def train_model():
    ...
    model = ConditionalUNet(
        ...
        base_filters=128,
        circular_padding=True # V2: Enable spherical padding
    )
    ...
    # V2: Pre-compute Latitude Weights for Loss
    # Shape: (1, 1, H, 1) for broadcasting against (B, C, H, W)
    # Lats: -90 to 90 (181 points)
    lats = torch.linspace(-90, 90, config["image_size"][0], device=accelerator.device)
    weights = torch.cos(torch.deg2rad(lats))
    lat_weights = weights.view(1, 1, -1, 1)
    ...
    for epoch in range(start_epoch, config["num_epochs"]):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                # V2: Latitude-Weighted MSE Loss
                loss = (F.mse_loss(noise_pred, noise, reduction='none') * lat_weights).mean()
