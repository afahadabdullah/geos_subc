#!/bin/bash
#SBATCH -J SA_test_v9_2
#SBATCH -o ml_output_flowmulti_v9_2_sa_55e100e_0n40n_noisectx_t2mres/test_%j.log
#SBATCH -e ml_output_flowmulti_v9_2_sa_55e100e_0n40n_noisectx_t2mres/test_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🧪 South Asia Multi-v9.2 full test started at $(date) on $(hostname)"
...
CONFIG_PATH="${SA_TEST_CONFIG:-ml_model/config_flow_multiv9_2.yaml}"
...
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" \
    ml_model/train_flow_multiv9_2.py \
    --config "$TEST_CONFIG" \
    --test \
    --ckpt "$CKPT_ARG"

echo "🏁 South Asia Multi-v9.2 full test finished at $(date)"
