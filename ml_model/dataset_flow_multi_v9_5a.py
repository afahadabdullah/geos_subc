...
# v9.5a GEOS conditioning order. The first axis of x_geos is now a statistic
# axis rather than the legacy singleton "member" axis.
GEOS_CONDITION_STATISTICS = ("mean", "std", "q10", "q90", "member_count")
GEOS_MEMBER_COUNT_REFERENCE = 4.0
...
class S2SHybridDataset(Dataset):
    ...
    Inputs:
        ...
        - X_geos: (5 statistics, 2 variables, 4 leads, H, W)
          Statistics are mean, std, q10, q90, and normalized member count.
    ...
    def _load_sample(self, idx, handles=None, cached_common=None, return_common_only=False):
        ...
        if cached_common is not None:
            ...
            geos_pr_finite = torch.isfinite(geos_pr_tensor)
            geos_tas_finite = torch.isfinite(geos_tas_tensor)
            ...
            if geos_pr_tensor.ndim == 3: # missing M
                 ...
                 geos_pr_finite = geos_pr_finite.unsqueeze(0)
                 geos_tas_finite = geos_tas_finite.unsqueeze(0)

            member_available = (
                geos_pr_finite.flatten(start_dim=2).any(dim=2)
                & geos_tas_finite.flatten(start_dim=2).any(dim=2)
            )
            member_count_by_lead = member_available.sum(dim=0).float() # [L]
            ...
            # The T2M residual target remains exactly the v9.3 target:
            # observed T2M minus the target-lead GEOS ensemble mean.
            ...
            # v9.5a conditions on target-lead ensemble distribution summaries.
            # For deterministic inputs (M=1), unbiased=False gives exactly zero
            # spread and q10/q90 equal the deterministic member.
            geos_pr_std = geos_pr_tensor.std(dim=0, unbiased=False)
            geos_tas_std = geos_tas_tensor.std(dim=0, unbiased=False)
            geos_pr_q10 = torch.quantile(geos_pr_tensor, 0.10, dim=0)
            geos_tas_q10 = torch.quantile(geos_tas_tensor, 0.10, dim=0)
            geos_pr_q90 = torch.quantile(geos_pr_tensor, 0.90, dim=0)
            geos_tas_q90 = torch.quantile(geos_tas_tensor, 0.90, dim=0)
            member_count_field = member_count_by_lead.view(-1, 1, 1).expand_as(
                geos_pr_mean
            )

            # Shape [S=5 statistics, C=2 variables, L=4, H, W].
            geos_cond_tensor = torch.stack(
                [
                    geos_mean_tensor,
                    torch.stack([geos_pr_std, geos_tas_std], dim=0),
                    torch.stack([geos_pr_q10, geos_tas_q10], dim=0),
                    torch.stack([geos_pr_q90, geos_tas_q90], dim=0),
                    torch.stack([member_count_field, member_count_field], dim=0),
                ],
                dim=0,
            )
            ...
            if self.normalize and self.bounds is not None:
                ...
                def min_max_scale(value, vmin, vmax):
                    return (
                        2.0
                        * (torch.clamp(value, vmin, vmax) - vmin)
                        / (vmax - vmin + 1e-6)
                        - 1.0
                    )

                # Mean and quantiles use the same physical mappings as the
                # original v9.3 GEOS mean channels.
                for statistic_index in (0, 2, 3):
                    geos_cond_tensor[statistic_index, 0] = min_max_scale(
                        geos_cond_tensor[statistic_index, 0], g_min, g_max
                    )
                    geos_cond_tensor[statistic_index, 1] = min_max_scale(
                        geos_cond_tensor[statistic_index, 1], tas_min, tas_max
                    )

                # Spread is nonnegative. Log scaling retains small but useful
                # member disagreement while bounding the channel in [0, 1].
                pr_spread_range = max(float(g_max) - float(g_min), 1.0)
                t2m_spread_range = tas_max - tas_min
                geos_cond_tensor[1, 0] = torch.clamp(
                    torch.log1p(geos_cond_tensor[1, 0])
                    / np.log1p(pr_spread_range),
                    min=0.0,
                    max=1.0,
                )
                geos_cond_tensor[1, 1] = torch.clamp(
                    torch.log1p(geos_cond_tensor[1, 1])
                    / np.log1p(t2m_spread_range),
                    min=0.0,
                    max=1.0,
                )

                # Count is repeated for PR/T2M so every variable has the same
                # five-statistic layout. M=1 remains distinct from M=4.
                normalized_count = torch.clamp(
                    torch.log1p(member_count_field)
                    / np.log1p(GEOS_MEMBER_COUNT_REFERENCE),
                    min=0.0,
                    max=1.0,
                )
                geos_cond_tensor[4, 0] = normalized_count
                geos_cond_tensor[4, 1] = normalized_count
            ...
            cached_common = {
                ...
                "geos_member_count_raw": member_count_by_lead,
            }
        ...
        return {
            ...
            "geos_member_count_raw": cached_common["geos_member_count_raw"],
