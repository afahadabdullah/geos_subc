...
# Local Modules
from dataset_flow_finalv1_global import (
...
from flow_matching_finalv1_global import FlowMatchingModel, CustomFlowMatcher
from static_geography_flow_finalv1_global import STATIC_CHANNEL_NAMES, load_or_build_static_geography
import noise_utils_finalv1_global
...
def select_geos_condition(x_geos, lead_idx, lead_selective_geos=True):
    ...
    flow_finalv1_global uses only the GEOS forecast lead matching each training sample. Set
    ...
    if t2m_target_mode == "geos_residual":
        if residual_bounds is None:
            raise KeyError(
                ...
                "in the stats file. Run calculate_global_stats_flow_finalv1_global.py."
            )
    ...
    for b_idx, batch in enumerate(val_loader):
        ...
        if validation_noise_cache is not None and cache_key in validation_noise_cache:
            ...
            if use_eof_lhs_noise:
                ...
                noise_expanded = noise_utils_multi_finalv1_global.mix_noise_with_random_multi(
        ...
        if print_validation_noise_diag and not did_print_noise_diag and accelerator.is_main_process:
            ...
            noise_utils_multi_finalv1_global.print_noise_channel_stats(noise_expanded.float(), prefix="Val Noise")
            noise_utils_multi_finalv1_global.print_noise_channel_stats(p_x1_expanded.float(), prefix="Val ODE Output")
...
def train(args, accelerator):
    ...
    output_dir = config.get("output_dir", "ml_output_flow_finalv1_global")
    ...
    stats_filename = config.get("stats_file", "flow_finalv1_global_stats.pt")
    ...
    if not os.path.exists(stats_file):
        raise FileNotFoundError(
            f"CRITICAL: {stats_file} missing. Please run calculate_global_stats_flow_finalv1_global.py first!"
        )
    ...
    # ``var_forced`` is the canonical flow_finalv1_global switch for entering variance-only
    ...
    if configured_geos_statistics != GEOS_CONDITION_STATISTICS:
        raise ValueError(
            "flow_finalv1_global requires geos_condition_statistics="
    ...
    if not np.isclose(
        ...
        raise ValueError(
            "flow_finalv1_global dataset and config disagree on geos_member_count_reference: "
    ...
    if accelerator.is_main_process:
        ...
        if bool(config.get("plot_topography", True)):
            print(
                f"   [Topo Diagnostic]   : "
                f"{os.path.join(output_dir, 'topography_flow_finalv1_global_diagnostic.png')}"
            )
        print(
    ...
    if accelerator.is_main_process:
        ...
        print(f"   --- flow_finalv1_global Conditioning Upgrades ---")
        print(
    ...
    output_dir = config.get("output_dir", "ml_output_flow_finalv1_global")
    os.makedirs(output_dir, exist_ok=True)
    log_file = os.path.join(output_dir, "training_log_flow_finalv1_global.csv")
    component_log_file = os.path.join(output_dir, "training_components_flow_finalv1_global.csv")
    metrics_log_file = os.path.join(output_dir, "validation_metrics_flow_finalv1_global.csv")
    ...
    try:
        nao_lookup = noise_utils_finalv1_global.parse_nao_index(os.path.join(config["data_dir"], "norm.daily.nao.index.b500101.current.ascii"))
        oni_lookup = noise_utils_finalv1_global.parse_oni_index(os.path.join(config["data_dir"], "oni.ascii.txt"))
    ...
    if accelerator.is_main_process:
        ...
        # 1. Reverse-normalize the flow_finalv1_global GEOS ensemble-mean PR channel.
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if accelerator.is_main_process:
            ...
            if not crps_loss and train_vel_var_steps > 0:
                print(
                    "   flow_finalv1_global components "
