#!/bin/bash
#SBATCH -J gen_mv1_pure_hc
#SBATCH -o ml_output_flowmulti/gen_multiv1_pure_1999_2019_%j.log
#SBATCH -e ml_output_flowmulti/gen_multiv1_pure_1999_2019_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 Pure-noise multi-v1 hindcast generation job started at $(date) on $(hostname)"
...
mkdir -p dataprocess/gen_multiv1_pure_1999_2019
...
echo "🎯 Generating pure-noise 4-member hindcasts for ${START_YEAR}-${END_YEAR}"
...
accelerate launch --num_processes 1 --mixed_precision fp16 \
    ml_model/generate_multiv1_pure_1999_2019.py \
    --config "$CONFIG_PATH" \
...
if [ "$status" -ne 0 ]; then
    echo "❌ Pure-noise hindcast generation failed with exit code $status. Not auto-resubmitting."
    exit "$status"
fi

all_done=1
for year in $(seq "$START_YEAR" "$END_YEAR"); do
    final_store="dataprocess/gen_multiv1_pure_1999_2019/${year}.zarr"
...
if [ "$all_done" -eq 1 ]; then
    ...
    next_job_id=$(ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch --parsable ml_model/submit_generate_multiv1_pure_1999_2019.sh")
...
echo "🏁 Pure-noise multi-v1 hindcast generation job finished at $(date)"
