#!/usr/bin/env python3
"""
Generate South Asia flow_finalv1.0 ensemble forecasts exactly like v9 test mode.

Unlike generate_forecast_zarr_flow_finalv1_0_sa.py, this script creates the full
EOF-LHS/random mixed ensemble noise for each batch before chunking the ODE solve.
That matches train_flow_finalv1_0.run_full_test_suite_multi and avoids changing
...
from compare_noise_flow_finalv1_0_sa import (
    decode_multi,
    load_noise_context,
    resolve_checkpoint,
...
from dataset_flow_finalv1_0 import S2SHybridDataset, resolve_target_domain
from flow_matching_finalv1_0 import CustomFlowMatcher, FlowMatchingModel
from train_flow_finalv1_0 import (
    euler_solve_chunked,
...
DEFAULT_OUT_DIR = "dataprocess/gen_flow_finalv1_0_sa_55e100e_0n40n_junjul_testmode_e100_s50"


def parse_args():
    parser = argparse.ArgumentParser(description="Generate flow_finalv1.0 SA forecasts to yearly Zarr stores.")
...
def write_year(year, args, config, model, flow_matcher, device, checkpoint_path, checkpoint_meta, noise_context, deadline=None):
    ...
    dataset = S2SHybridDataset(
        ...
        normalize=True,
        preload=False,
        stats_file=config.get("stats_file", "flow_finalv1_0_sa_55e100e_0n40n_global_local_stats.pt"),
    ...
    seen_inits = 0
    pbar = tqdm(loader, desc=f"Generate flow_finalv1.0 SA {year}")
...
def main():
    ...
    print("\n" + "=" * 88)
    print("South Asia flow_finalv1.0 forecast Zarr generation (test-mode equivalent)")
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        if not completed:
            ...
            break
    if all_complete:
        print("✅ Requested flow_finalv1.0 SA Zarr generation range is complete.")
    else:
        print("⏸️ Requested flow_finalv1.0 SA Zarr generation range is incomplete and can be resumed.")
