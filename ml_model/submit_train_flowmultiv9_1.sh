#!/bin/bash
#SBATCH -J SA_flow_v9_1
#SBATCH -o ml_output_flowmulti_v9_1_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
#SBATCH -e ml_output_flowmulti_v9_1_sa_55e100e_0n40n_noisectx_t2mres/flow_%j.log
...
echo "🚀 South Asia Multi-v9.1 local/global residual-T2M training job started at $(date) on $(hostname)"
...
if [ "${CLEANUP_OLD_PROCS:-0}" = "1" ]; then
    ...
    pkill -9 -u "$USER" -f "train_flow_multiv9_1.py" || true
    pkill -9 -u "$USER" -f "accelerate.*train_flow_multiv9_1.py" || true
    sleep 3
else
...
CONFIG_PATH="ml_model/config_flow_multiv9_1.yaml"
...
if [ "$CONFIG_OUTPUT_DIR" != "ml_output_flowmulti_v9_1_sa_55e100e_0n40n_noisectx_t2mres" ] || [ "$CONFIG_TARGET_DOMAIN" != "south_asia" ]; then
...
echo "🔥 Launching Flow Matching Multi-Target v9.1 Training (SA target, local/global predictors, residual T2M)..."
accelerate launch --num_processes 1 --mixed_precision "$MIXED_PRECISION" ml_model/train_flow_multiv9_1.py --config "$CONFIG_PATH" \
    --epochs-per-run 20
...
if [ -f "$CKPT_FILE" ]; then
    ...
    if [ "$CURRENT_EPOCH" -lt "$MAX_EPOCHS" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" "cd $PWD && sbatch ml_model/submit_train_flowmultiv9_1.sh"
    else
...
echo "🏁 South Asia Multi-v9.1 local/global residual-T2M training job finished at $(date)"
