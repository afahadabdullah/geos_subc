#!/bin/bash
#SBATCH -J GLOBAL_finalv1_tgen
#SBATCH -o ml_output_flow_finalv1_global_noisectx_t2mres/generate_testmode_%j.log
#SBATCH -e ml_output_flow_finalv1_global_noisectx_t2mres/generate_testmode_%j.log
#SBATCH -p gh-dev
#SBATCH -N 1
#SBATCH -n 1
#SBATCH -t 02:00:00
...
echo "🚀 Global flow_finalv1_global test-mode-equivalent Zarr generation started at $(date) on $(hostname)"
...
CONFIG_PATH="${GLOBAL_TESTGEN_CONFIG:-ml_model/config_flow_finalv1_global.yaml}"
START_YEAR="${GLOBAL_TESTGEN_START_YEAR:-2021}"
END_YEAR="${GLOBAL_TESTGEN_END_YEAR:-2024}"
SKIP_YEARS="${GLOBAL_TESTGEN_SKIP_YEARS:-}"
MONTHS="${GLOBAL_TESTGEN_MONTHS:-6,7}"
NUM_ENSEMBLE="${GLOBAL_TESTGEN_ENSEMBLE:-100}"
NUM_STEPS="${GLOBAL_TESTGEN_STEPS:-50}"
CHECKPOINT="${GLOBAL_TESTGEN_CHECKPOINT:-best_flow_ckpt.pt}"
OUT_DIR="${GLOBAL_TESTGEN_OUT_DIR:-dataprocess/gen_flow_finalv1_global_junjul_testmode_e100_s50}"
BATCH_SIZE="${GLOBAL_TESTGEN_BATCH_SIZE:-8}"
NUM_WORKERS="${GLOBAL_TESTGEN_NUM_WORKERS:-1}"
ENSEMBLE_CHUNK="${GLOBAL_TESTGEN_ENSEMBLE_CHUNK:-30}"
ODE_BATCH_SIZE="${GLOBAL_TESTGEN_ODE_BATCH:-120}"
MEMBER_CHUNK="${GLOBAL_TESTGEN_MEMBER_CHUNK:-10}"
SEED="${GLOBAL_TESTGEN_SEED:-1234}"
OVERWRITE="${GLOBAL_TESTGEN_OVERWRITE:-0}"
CLEAN_OUTPUT="${GLOBAL_TESTGEN_CLEAN_OUTPUT:-0}"
MAX_RUNTIME_MINUTES="${GLOBAL_TESTGEN_MAX_RUNTIME_MINUTES:-100}"
AUTO_RESUBMIT="${GLOBAL_TESTGEN_AUTO_RESUBMIT:-1}"
...
if [ "$CONFIG_TARGET_DOMAIN" != "global" ]; then
    echo "❌ Refusing non-global config: target_domain=$CONFIG_TARGET_DOMAIN"
    exit 1
fi
...
python ml_model/generate_forecast_zarr_flow_finalv1_global_testmode.py \
    --config "$CONFIG_PATH" \
...
if [ -n "$MISSING_YEARS" ]; then
    ...
    if [ "$AUTO_RESUBMIT" = "1" ] || [ "$AUTO_RESUBMIT" = "true" ] || [ "$AUTO_RESUBMIT" = "TRUE" ]; then
        ...
        ssh -o StrictHostKeyChecking=no "$SUBMIT_TARGET" \
            "cd $PWD && DATA_DIR_OVERRIDE='$DATA_DIR_OVERRIDE' GLOBAL_TESTGEN_CONFIG='$CONFIG_PATH' GLOBAL_TESTGEN_START_YEAR='$START_YEAR' GLOBAL_TESTGEN_END_YEAR='$END_YEAR' GLOBAL_TESTGEN_SKIP_YEARS='$SKIP_YEARS' GLOBAL_TESTGEN_MONTHS='$MONTHS' GLOBAL_TESTGEN_ENSEMBLE='$NUM_ENSEMBLE' GLOBAL_TESTGEN_STEPS='$NUM_STEPS' GLOBAL_TESTGEN_CHECKPOINT='$CHECKPOINT' GLOBAL_TESTGEN_OUT_DIR='$OUT_DIR' GLOBAL_TESTGEN_BATCH_SIZE='$BATCH_SIZE' GLOBAL_TESTGEN_NUM_WORKERS='$NUM_WORKERS' GLOBAL_TESTGEN_ENSEMBLE_CHUNK='$ENSEMBLE_CHUNK' GLOBAL_TESTGEN_ODE_BATCH='$ODE_BATCH_SIZE' GLOBAL_TESTGEN_MEMBER_CHUNK='$MEMBER_CHUNK' GLOBAL_TESTGEN_SEED='$SEED' GLOBAL_TESTGEN_OVERWRITE='0' GLOBAL_TESTGEN_CLEAN_OUTPUT='0' GLOBAL_TESTGEN_MAX_RUNTIME_MINUTES='$MAX_RUNTIME_MINUTES' GLOBAL_TESTGEN_AUTO_RESUBMIT='$AUTO_RESUBMIT' sbatch ml_model/submit_generate_forecast_zarr_flow_finalv1_global_testmode.sh"
    else
...
echo "🏁 Global flow_finalv1_global test-mode-equivalent Zarr generation finished at $(date)"
