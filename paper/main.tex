...
\begin{abstract}
Skillful subseasonal forecasts of precipitation and near-surface temperature are needed for hydrologic risk, agriculture, energy planning, public health, and disaster preparedness, yet raw dynamical ensembles retain regional biases and imperfect probabilistic calibration at weeks 1--4. We introduce \methodname{}, a global machine-learning framework that transforms \modelname{} (\modelshort{}) subseasonal ensemble guidance into stochastic, spatially coherent ensemble forecasts of weekly precipitation and 2\,m temperature. The central design choice is to generate ensemble members from a physically informed stochastic prior rather than from isotropic Gaussian noise: initial perturbations combine EOF structures conditioned on MJO, NAO, ENSO, and lead-week state with random support and variance-tempered spread control. A lead-aware flow-matching U-Net is conditioned on \modelshort{} ensemble summary statistics, recent observed land--ocean--atmosphere predictors, static geography, seasonal phase, and lead time; it predicts precipitation directly and 2\,m temperature as a residual relative to the target-lead \modelshort{} ensemble mean. Because probabilistic members are produced by integrating the learned velocity field from these structured initial states, the flow refines physically plausible large-scale covariance patterns instead of constructing all spatial organization from white noise. On the held-out 2021--2023 evaluation, \methodname{} reduces all-case CRPS relative to raw \modelshort{} by 32.2\% for precipitation and 34.0\% for 2\,m temperature globally, with comparable land-only gains (32.3\% and 32.7\%) and positive CRPS and RMSE skill in every season--lead cell. A controlled ablation isolates the contribution of the prior itself: at fixed checkpoint, ensemble size, and ODE solver, replacing Gaussian initialization with the regime-conditioned EOF prior improves precipitation CRPS skill from 24.9\% to 28.4\% and temperature CRPS skill from 41.7\% to 43.2\%. Member-count experiments show that skill converges by roughly 20--30 generated members, supporting the 90-member production ensemble. Skill extends to the tails: on an observed-95th-percentile event subset, CRPS improves by 13.8\% (precipitation) and 26.8\% (temperature) and calibrated Brier skill gains are positive for both variables, while case studies of the July 2022 UK heatwave and January 2023 California atmospheric rivers show sharper, better-placed tail probabilities at week-4 lead. We also document where the approach cannot help: when the dynamical ensemble carries no signal for an event, as for the August 2022 Pakistan floods, the refinement does not recover it.
\end{abstract}
...
\end{equation}
where $z$ contains recent observed predictors and $s$ contains static geography and calendar information. The dynamical model provides a physically informed prior state; machine learning provides spatially coherent, observation-consistent distributional corrections. An immediate corollary, examined explicitly in Sect.~\ref{sec:results-events}, is that the refinement inherits the predictability limits of its conditioning information: it can sharpen and relocate probability mass that the dynamical ensemble and observed precursors support, but it cannot create signal where none exists.
...
Ensemble-generation properties are evaluated separately: member-count subsampling experiments quantify how CRPS and RMSE skill converge with the number of generated members and how the spread/RMSE ratio evolves, with member-resampling uncertainty bands from repeated random subsets.
...
\begin{table}[t]
\centering
\caption{Headline all-case skill on the held-out 2021--2023 evaluation. CRPS and RMSE are negatively oriented ($\downarrow$: lower is better); the skill column is the percentage score reduction relative to \modelshort{} (Eq.~\ref{eq:skill}), so positive values always denote improved skill. Units are mm\,day$^{-1}$ (PR) and K (T2M).}
...
\caption{Noise-prior ablation at fixed checkpoint, ensemble size, ODE solver, and conditioning (2021 full year, 52 batches). (a) All-grid CRPS improvement (\%) relative to raw \modelshort{} for the pure Gaussian prior and the regime-conditioned EOF-LHS prior ($\rho_{\mathrm{PR}}=0.25$, $\rho_{\mathrm{T2M}}=0.08$; $\beta_{\mathrm{PR}}=0.45$, $\beta_{\mathrm{T2M}}=0.03$), for precipitation and 2\,m temperature. (b) Relative CRPS reduction of the physically informed prior with respect to the Gaussian neural ensemble, isolating the contribution of the prior after architecture and dynamical conditioning are held fixed.}
\label{fig:noise-ablation}
\end{figure}

\subsection{Ensemble size, convergence, and dispersion}
\label{sec:results-ensemble}

A stochastic generator must also be characterized as an ensemble system: how many members are needed, and how well does ensemble spread represent forecast uncertainty? Figure~\ref{fig:member-convergence} shows CRPS skill, RMSE skill, and the spread/RMSE ratio as functions of the number of generated members on the extreme-event subset at weeks 3 and 4, with shading spanning the 5th--95th percentile over repeated random member subsets.

Three features are relevant for practice. First, a single generated member scores worse than the raw \modelshort{} ensemble---as it must, since one sample cannot represent a distribution---so the probabilistic gains reported in this paper are genuinely ensemble properties rather than deterministic-map properties. Second, skill rises steeply with ensemble size and saturates by roughly 20--30 members, with the 90-member production configuration comfortably inside the plateau; member-sampling uncertainty at 90 members is small compared with the skill signal. Third, the spread/RMSE ratio saturates near 0.7 for both variables rather than at the ideal value of one, indicating that the generated ensemble remains moderately underdispersive on the extreme subset even with variance tempering. This residual spread deficit is an honest limitation of the current sampler and is consistent with the reliability-slope analysis of Sect.~\ref{sec:results-extremes}; we return to it in Sect.~\ref{sec:limitations}.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig5_member_convergence}
\caption{Ensemble-size convergence and dispersion on the extreme-event subset at weeks 3 and 4. (a,~d) CRPS skill and (b,~e) RMSE skill versus raw \modelshort{} as a function of the number of generated members, for precipitation (top) and 2\,m temperature (bottom); (c,~f) the \methodname{} spread/RMSE ratio (dashed line: perfect dispersion). Shading spans the 5th--95th percentile over repeated random member subsets. Skill saturates by roughly 20--30 members and the spread/RMSE ratio plateaus near 0.7, indicating moderate residual underdispersion.}
...
The extreme-event results must be interpreted with the logic of probability forecasting. At subseasonal leads a well-calibrated ensemble mean should be smoother than individual outcomes; that is a feature, not a failure. The correct verification question for rare events is whether the forecast distribution assigns elevated, spatially relevant probability to the event and whether the observed value falls within a plausible forecast tail. On both the systematic $q_{0.95}$ subset and the named case studies, \methodname{} improves on the raw dynamical ensemble under exactly these distribution-based criteria, while the reliability-slope analysis and the spread/RMSE ratio of $\approx0.7$ jointly indicate that residual overconfidence, not signal absence, is the dominant remaining calibration error.

\section{Limitations and Future Work}
\label{sec:limitations}

Five limitations qualify these results. First, the generated ensemble remains moderately underdispersive: the spread/RMSE ratio plateaus near 0.7 on the extreme subset (Fig.~\ref{fig:member-convergence}), and raw exceedance probabilities are overconfident, so downstream users should apply the reliability calibration evaluated here or equivalent. Second, uncertainty quantification is only partial: member-resampling bands are provided for the ensemble-size experiments, but case-level block-bootstrap confidence intervals for the headline and ablation scores remain to be added. Third, the structured stochastic prior is partly heuristic: EOF bases and teleconnection regimes are physically motivated, but the mixture weights $\rho_q$ are manually configured rather than learned end to end. Fourth, the variance mechanism adjusts the initial spread only and does not inject stochasticity continuously along the ODE trajectory; an SDE sampling path is the natural remedy for both this and the dispersion deficit. Fifth, precipitation remains hardest at weeks 3--4, where heavy rainfall is intermittent, spatially displaced, and often tied to mesoscale or tropical-cyclone processes that are not deterministic targets at subseasonal lead---and, as the Pakistan case shows, no refinement can compensate when the dynamical prior misses the event entirely.
...
The training, inference, and evaluation code, the frozen evaluation products used for all tables and figures, and the figure-generation scripts will be made publicly available upon publication. \modelshort{} forecasts are available from the S2S archive \citep{Vitart2017}; GPCP, ERA5, OISST, Copernicus salinity, and C3S soil-moisture data are available from their respective providers.
